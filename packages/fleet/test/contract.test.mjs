...
describe('review remains independent within the same task', () => {
  ...
  it('blocks P1, keeps P2/P3 advisory and preserves stable finding ids', () => {
    ...
    const p2 = { id: 'naming', severity: 'P2', type: 'maintainability', effort: 'small', detail: 'naming' };
  ...
  it('T32：P2/P3 必须带 type（枚举），P1 不用；带了脏位置也拒', () => {
    ...
    const good = { id: 'c', severity: 'P2', type: 'perf', effort: 'small', file: 'x.mjs', line: 3, detail: 'd' };
    ...
    assert.equal(judgeReview(normalizeTask(input()), HEAD, { ...review(), findings: [{ ...good, effort: undefined }] }).state, 'unscanned', 'P2 有 type 没 effort 必须拒');
    assert.equal(judgeReview(normalizeTask(input()), HEAD, { ...review(), findings: [{ ...good, effort: 'huge' }] }).state, 'unscanned', '脏 effort 必须拒');
...
describe('retry classification does not create new task cards', () => {
  it('authentication needs repair, throttling waits, transport loss is not a code-review failure', () => {
    ...
    // T39 ①：本步超预算（Temporal 的 startToCloseTimeout 到点）= 阶段停滞，单独一态；
    // 它既不是「没查成」，也不该拿重试去刷同一堵墙（预算已经烧完）。
    assert.equal(classifyStepFailure({ code: 'TIMEOUT' }), 'stall', '本步超预算判停滞');
    assert.equal(classifyStepFailure({ code: 'STALL' }), 'stall');
    assert.equal(classifyStepFailure({ code: 'DEADLINE_EXCEEDED' }), 'retryable', '我们自己的未知态截止仍是可重试');

...
const childPath = join(here, process.env.DAO_SYNC_BLOCK_TREE ? 'sync-block-tree.mjs' : 'sync-block.mjs');
...
if (pidFile) {
  writeFileSync(pidFile, JSON.stringify({ owner: process.pid, middle: child.pid }));
}
setInterval(() => {}, 1000);

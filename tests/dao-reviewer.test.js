...
describe('dao 审官与完工', () => {
  ...
  it('#564 label 自动打：dispatch 记 issue + pr-sync-labels 合并侧同步到 PR', async (t) => {
    ...
    await t.test('dao.mjs mirasim 派工在起会话前 stampIssueLabels（#1205），身份 marshal', () => {
      ...
      assert.match(mira, /stampIssueLabels\(/, '缺 type/ 时派工前补标，否则 pr-sync-labels 拒合并');
      assert.match(mira, /role:\s*'marshal'/, '打标身份必须 marshal');
    });

...
describe('执行体策略读取', () => {
  ...
  it('仓内真表读得出来，且不是 0 条（零样本报红）', async () => {
    ...
    // #880 / #982：claude 云端 relay；未登记家族（grok）拒派，不把落地通道 gw 当成厂商族。
    ...
    assert.equal(grok.ok, false, 'grok 未登记家族必须拒派（#982），不许被 provider=gw 带去 pi');
    assert.equal(grok.family, 'grok');
    assert.match(grok.error, /拒派/);
  });
});
...
describe('worker-start --executor mirasim 超长 --spec 结构化拒派（#884 P1#3，四轮）', () => {
  ...
  it('超长 --spec → JSON 拒派 + 退出 1，绝不甩 Node 栈', () => {
    ...
    assert.ok(out.injectGate && out.injectGate.ok === false, '回执要点名是注入闸拒的');
    assert.equal(r.status, 1, '拒派要非零退出，否则脚本调用方看不出被拒');
  });
...
describe('判别实验：未登记家族拒派 / claude·codex 放行（#982）', () => {
  ...
  it('未登记家族 grok-4.6 → 结构化拒派，一个会话都不许起', () => {
    const r = dry('grok-4.6');
    const out = JSON.parse(String(r.stdout || '').trim());
    assert.equal(out.ok, false, 'grok 未登记却放行 = #982 硬边界破了');
    assert.match(String(out.error || ''), /拒派/);
    assert.equal(out.route && out.route.family, 'grok');

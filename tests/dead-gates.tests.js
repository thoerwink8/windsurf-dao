...
// @dao-test-tier: env
//
// 分层：本文件一小撮断言对机器级可变状态（真实 live settings / cc-switch GUI 库 / 指向共享
// 主仓的命令）做不变量断言，故标 env——它不制造污染，是被别人的正常活动污染。被 defer 的是
// ⑪/⑪.5 共 2 组；默认层 DEFER 是 2 或 3（hook 跳过死闸检测时 ⑫① 可达性那条也 defer），
// **别把 DEFER 当常量写进断言**（run-tests 只要求 `DEFER>0`）。真实语料那一半由 SessionStart
// hook（dao-scaffold-check.js 每会话三层跑 check-dead-gates，⑪ 核心断言即它在真实语料上求值
// 的东西）接管，本文件保障「检测器本身没坏」。分层过程史见 docs/evolution/comment-archive-202608.md §C8。
// ⚠ 跑 --env 要求串行：没有别的官在跑测试 · cc-switch GUI 没在写库 · 没人在改 settings.json。
//
// 断言分两类，缺一不可：
//   正控 —— 已知死闸必须被检出且**必须指名**（只报个数字，人无从下手）
//   负控 —— 活的闸/无法核验的闸/孤儿 hook 一律不许判红（生下来就吵的检查会被静音，静音与不存在等价）
// 另有一组钉**自检半边**：构造「普查看得见、结构化遍历看不见」的样本断言 `selfcheck=fail`
// —— 那一半若失灵，上面所有「零死闸」都不可信。
// 夹具全落 `_tmp/dead-gates-tests/<case>/`：live.json / snap/settings.json / hooks/ /
// providers-db/*.db（⑬ 用临时 sqlite 库，从不碰真库）；两层刻意用不同路径写法（绝对 vs
// `${PROJECT_ROOT}` 占位符）顺带钉住占位符还原。最后一例是真实语料自跑，刻意留着——
// 合成夹具证明不了「它在真实数据上跑得动」。

...
const { deriveState, pendingAction, pickReviewer, orderedSignals, isInstitutional, awaitingShuaiReason, mergeGate, ciState, mergeableVerdict, greenCommitVerdict, extractPrsFromSpec, selectDeliveryDispatch, runCmd, taskIdFromTaskCreate, handleFromWorkerShow, worktreeIdFromWorktreeCreate, terminalHandleFromTerminalCreate, dispatchIdFromDispatchShow } = require("../scripts/flow.mjs");
...
{
  // 样本1（审官变异固化，fake-loop：构造态端到端路径，README 注明非真实分布）：dispatched + 同 worktree 历史 completed → 正常返工照常注入
  ...
  // 样本3（真实回抄：本 PR 工位行 113/125/104）：多条 dispatched（真歧义）→ 待帅转交，不猜
  ...
  // 样本2（真实回抄：存档行 54/55）：多条 completed 无在岗 → worker-list 无时间字段不猜顺序 → 待帅转交
...
console.log("\n=== ㊶ 选择器纯函数单测（#497 第十四轮：真实存档不存在的组合用构造输入测逻辑分支，不伪装成语料）===");
{
  // 明确声明：输入为构造值，不代表真实分布（真实存档中 dispatched+failed、唯一 dispatched+历史 completed 为 0 条）。
  const mk = (dispatchId, status, wt) => ({ dispatchId, worktreeId: wt, dispatchStatus: status });
  const W = "wt::w1";
  // ⑦ 唯一 dispatched → 选它
  check("⑦ 唯一 dispatched → 选它", (() => { const r = selectDeliveryDispatch([mk("d1", "dispatched", W)], W); return r && r.dispatchId === "d1"; })(), "");
  // ⑥ 唯一 completed → 选它（闲置可投递）
  check("⑥ 无在岗、唯一 completed → 选它", (() => { const r = selectDeliveryDispatch([mk("c1", "completed", W)], W); return r && r.dispatchId === "c1"; })(), "");
  // ⑤ 仅 failed → 不选（null 待帅转交）
  check("⑤ 仅 failed → null（failed 不是投递目标）", selectDeliveryDispatch([mk("f1", "failed", W)], W) === null, "");
  // 构造态组合：dispatched + completed 历史（真实存档 0 条）→ dispatched 优先
  check("① dispatched + completed 历史 → 选 dispatched（在岗优先）", (() => { const r = selectDeliveryDispatch([mk("c1", "completed", W), mk("d1", "dispatched", W)], W); return r && r.dispatchId === "d1"; })(), "");
  // 构造态组合：dispatched + failed（真实存档 0 条）→ failed 排除，选 dispatched
  check("④ dispatched + failed → failed 排除，选 dispatched", (() => { const r = selectDeliveryDispatch([mk("f1", "failed", W), mk("d1", "dispatched", W)], W); return r && r.dispatchId === "d1"; })(), "");
  // ③ 多条 dispatched → null（真歧义不猜）
  check("③ 多条 dispatched → null", selectDeliveryDispatch([mk("d1", "dispatched", W), mk("d2", "dispatched", W)], W) === null, "");
  // ② 多条 completed 无在岗 → null（无时间字段不猜顺序）
  check("② 多条 completed 无在岗 → null（无时间字段不猜顺序）", selectDeliveryDispatch([mk("c1", "completed", W), mk("c2", "completed", W)], W) === null, "");
  // 非本工位 worker 不入选

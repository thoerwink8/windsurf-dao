...
describe('grill-ai-pointers', () => {
  it('grill-ai 指针与记录契约（落点三态：SKIP / RED / GREEN）', async (t) => {
    ...
    await t.test('host/skills/grill-ai/SKILL.md 在', () => {
      assert.ok(grillAi !== null, 'host/skills/grill-ai/SKILL.md 在  →  指针起点缺失 ⇒ 本次等于没查');
    });
    await t.test('host/skills/grill-me/SKILL.md 在', () => {
      assert.ok(grillMe !== null, 'host/skills/grill-me/SKILL.md 在  →  指针落点缺失 ⇒ grill-ai 的五步法指向空气');
    });
    ...
    const g = (name, cond, detail) => gated.push({ name, cond, detail });
    ...
    if (!fs.existsSync(memDir)) {
      await t.test('判例落点：SKIP —— 本机无该项目 memory 目录 ⇒ 后续断言未执行（这是「没查成」，不是「查过没事」）',
        { skip: `本机无该项目 memory 目录（${memDir}）⇒ 判例落点与后续 ${gated.length} 条断言未执行` }, () => {});
    } else {
      await t.test(`判例 ${PRECEDENT}.md 在本机 memory 目录里`, () => {
        assert.ok(verdict !== null,
          `判例 ${PRECEDENT}.md 在本机 memory 目录里  →  判例落点缺失 ⇒ grill-ai 末尾的判例引用指向空气（${verdictPath}）`);
      });
      if (grillAi && grillMe && verdict) {
        for (const { name, cond, detail } of gated) {
          await t.test(name, () => {
            assert.ok(cond(), name + (detail ? '  →  ' + detail : ''));
          });
        }
      } else {
        await t.test('判例落点不可读 ⇒ 后续断言未执行（这是「没查成」，不是「查过没事」）',
          { skip: `判例落点不可读 ⇒ 后续 ${gated.length} 条断言未执行` }, () => {});
      }
    }
  });
});
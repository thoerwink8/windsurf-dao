...
// 外加：政策 YAML 解析、schema 闭集派生（= 设计 16 种 + #891 会话态 3 种 = 19 种）、配额覆盖、禁令门闩。
...
describe('dianjiangtai', () => {
  ...
  it('④ 事件写入工具', async (t) => {
    ...
    // 设计 16 种（docs/dianjiangtai-design.md A.1）+ #891 期一会话态 3 种。此处是「别悄悄加类型」的钉子，
    // 唯一权威仍是 schemas/events.schema.json；加类型要连这枚钉子一起改。
    const DESIGN_TYPES = ["job.opened", "job.dispatch", "job.meter", "job.handoff", "job.closed", "job.override", "job.explore", "attr.rule", "attr.llm", "attr.human", "attr.retract", "policy.patch", "sub.usage", "incident", "audit.bypass", "audit.stale", "session.state", "decision.pending", "decision.resolved"];
    await t.test('schema 闭集派生 = 19 种事件类型（设计 16 + #891 会话态 3）', () => {
      assert.ok(meta.closedSet.length === 19 && DESIGN_TYPES.every(t => meta.closedSet.includes(t)) && meta.closedSet.every(t => DESIGN_TYPES.includes(t)), 'schema 闭集派生 = 19 种事件类型（设计 16 + #891 会话态 3）  →  ' + meta.closedSet.join(","));
    });

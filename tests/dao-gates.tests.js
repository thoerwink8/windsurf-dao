...
// catch「路径打错字」这类最常见的接线错误。**数量断言是 4 不是 >=4**：GATES 是手维护的
// 清单（见 dao-gates.mjs 头注「已知的射程缺口」㈠），改闸清单时这个数字要跟着手改——
// 这是刻意的强断言，不是遗漏加固。
// （2026-08-11 重设计：gen-guarded-files --check 随派生物消灭退役，5→4。）
{
  ...
  check("从源码里静态抓出的 GATES 路径数 === 4（GATES 清单当前长度；改闸数要同步改这里）",
    realPaths.length === 4, "抓到 " + realPaths.length + " 条：" + JSON.stringify(realPaths));

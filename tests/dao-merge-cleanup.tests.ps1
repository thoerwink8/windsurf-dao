...
.DESCRIPTION
    ...
    **一格例外，照直写**：场景 11/12 要验的是「git 查询**失败**时脚本怎么办」，而真 git 在
    ...
    十六个场景，覆盖差集核验的两条安全路径、两类拒绝路径，以及幂等 / 前置校验 / DryRun：
     ...
     17. `-WorktreePath` 的目录被 rename 挪走（git 登记还在）⇒ 认树走分隔符归一化、配对
         校验照跑 ⇒ 退出码 1（#265 件 6；认不出树那一版这里是 0 且真的删）
     18. `git fetch` 命令本身失败 ⇒ 退出码 4（#260 件4 P16），worktree 与分支原样不动
     19. worktree 被 lock ⇒ `git worktree remove` 失败 ⇒ 退出码 4（#260 件4 P17）
     20. `git branch -d/-D` 命令本身失败 ⇒ 退出码 4（#260 件4 P18）
     21. `git pull --ff-only` 命令本身失败 ⇒ 退出码 4（#260 件4 P19）
    ...
    路径拼法，所以这个洞对它们结构性不可见。
    17 是 #265 件 6（复抗 X12）：目录被挪走与 16 的路径拼法是**两个独立变量**——16 已修、
    17 没修之前，挪走目录那一格仍会认不出登记 ⇒ 配对校验被跳过、打「视为已清理」假话。
    18–21 是 #260 件 4（退出码 4 的 7 个产出点只测到 1 个）：fetch / remove / branch-delete /
    pull 四个失败分支此前零样本，P16–P19 四个变体全绿。
    1/2 的 DELETE_FLAG 断言与 6 的报文断言是 #260 件 2 / 件 3 的落点（-d/-D 互换两向皆绿、
    前置校验 A 零样本）。

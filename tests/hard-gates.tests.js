...
//
// 被测对象是**一道会 exit 2 拦人的闸**，两侧代价都是真代价（漏报 ⇒ 禁令仍只是文字，文字禁令
// 实测遵守率 0%；误报 ⇒ 合法动作被拦死，甲类闸逃生阀只有用户设得了）⇒ 每闸**双向断言**：
// 违例必 exit 2 且 stderr 给得出合法路径；合法输入必 exit 0。只证明「能拦住」不算完成。
// mutation 做成**常驻断言**而非手工跑一次：「测试存在」≠「测试有判别力」，判据会被反复编辑——
// 把某一闸的判定**改坏**（写进 _tmp/ 副本，从不碰真文件），断言原 exit 2 的用例变 exit 0，
// 再断言真文件全程逐字节没动（canary 恒等）。任何一天有人把某条判据写成永假，这里会红。
//
// 已知不覆盖（照直写，别读成全覆盖）：
// · matcher 覆盖面只由 `--selfcheck` 自查，本文件只断言它的**输出形态**——
//   真实注册状态取决于用户 live settings.json，锚死会让测试随用户配置变红。
...
// · G6 与 dao-rhythm.js WAKEUP 信号的**跨文件判据一致性**（大小写/前缀规则两边同款）
//   由本文件 G6 一节直接断言（原 tests/dao-rhythm.tests.js 已随 tests 终局并入），
//   改任一侧都要看那一节。

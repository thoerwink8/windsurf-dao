...
describe('dispatch-launch（async-launch）', () => {
  ...
  it('#831 注入闸前移到热路：超长 --spec 当场非零，一棵树都不建', async (t) => {
    ...
    await t.test('dispatch-exec 绕过热路：orca 脊已删，当场拒', () => {
      assert.notEqual(exec.status, 0, 'exec 非零  →  ' + JSON.stringify(pExec).slice(0, 400));
      assert.equal(pExec.ok, false);
      assert.match(String(pExec.error || ''), /orca 已退役/);

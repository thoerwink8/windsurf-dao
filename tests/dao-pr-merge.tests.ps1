...
.DESCRIPTION
    ...
    最要紧的一条是场景 2（负控）：`gh pr merge --merge --delete-branch` 是**两个动作共用
    一个退出码**——合并半成功、删分支半失败时 gh 退出非 0 而 PR 其实已合，脚本若据此判
    「合并失败」硬停，第 6 步（prune + 实查远程 + 删分支）就不跑、远程分支残留。场景 2
    断言 **gh 退出 1、PR 实为 MERGED** 时脚本不停在第 5 步、走完第 6 步、远程分支清干净、
    退出码 0（`-DryRun` 结构上照不出这个缺口——它只打印不执行，那对分歧根本不发生）。

    git 用真的：fetch / merge / ls-remote / push --delete 全部打在一对**真的本地仓**上
    （`origin.git` 裸仓 + `work` 工作树），exit 4 那一档也是真的（裸仓设
    `receive.denyDeletes=true`，`push --delete` 真被拒）。

    gh 用桩：PATH 前置假 `gh`（沙盒 `bin/gh.cmd`），三形态里选它——被测脚本**一个字都
    不用改**，耦合建立在 gh 的**命令行契约**上（子命令、`--json` 字段名、退出码），
    `Get-Command gh` 与 `& gh` 同一条解析路径；参数注入（`-GhExe`）要在生产脚本开
    测试专用口子；环境变量开关让生产行为从此依赖环境变量，更糟。

    **桩的局限照直写**：它把 gh 今天的行为**冻**在这里了，gh 若改 `--json` 字段名或
    「合并成功但删分支失败」的退出码，本文件会继续全绿而现实已变；故断言尽量打在**脚本的
    可观察行为**上（走没走到第 6 步、远程分支还在不在、退出码是几），不打在内部变量上。

    gh 桩经 gh.cmd 转一道：`powershell -File x.ps1 pr view 42 --json …` 里的 `--json` 会被
    PowerShell 参数绑定器当成参数名去匹配 `param()`，匹配不上就报错；故 gh.cmd 把整串参数
    塞进环境变量 `DAO_GH_STUB_ARGS`，gh.ps1 自己拆（本仓被测参数没有含空格/引号的，
    简单按空白拆就够）。
...
.NOTES
    ...
    理由：那一段绕不开「本套此前为什么被标环境敏感层」，说清楚就免不了引用标记的字面形态——
    这个 help 块**落在 run-tests.mjs 标记扫描器的头 60 行窗口内**，散文里一个凑齐了行首井号的
    标记会被它当成真标记，把整套静默判出默认层且全仓零红（姊妹套 pr-body-scan 上已实测过）。
...
# issue #121 场景 12/13 需要真 node（被测脚本第 3.5 步调它）；找不到就跳过那两个场景而不是

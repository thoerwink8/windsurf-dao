...
# 与当时的判别力结论，已整段搬去 docs/evolution/comment-archive-202608.md
# §C1/§C2/§C3/§C4（四张表一表一个锚点；本文件不自带 mutation 跑法，见下面附四末尾那段）。
...
#
# issue #173 那批（记分板 docs/evolution/comment-archive-202608.md §C4）用的是**整行字面锚点**，
# 不是上面这些正则 —— 三条，含缩进：
...
# comment-archive-202608.md §C1/§C2/§C3/§C4）与本文件那条 18c 负控的说明都逐条引用了
...
#
# 判别力从哪来（数字取自实测，记分板见 docs/evolution/comment-archive-202608.md §C3）：把被测脚本末尾那句 `exit 2` 改回
...
# 判别力照直写（本批实测，记分板见 docs/evolution/comment-archive-202608.md §C4）：S19D / S19X 两个变体下 19d **红**；
...
# 换靶见 docs/evolution/comment-archive-202608.md §C3 的 **DR1** 变体（把 DryRun 块里那句
# `exit 0` 改成 `exit 2`）⇒ 20a 变红。（**这里此前写的是 `D1`，那是错的**：`D1` 是 §C2 里
#  「默认值」那个变体，记分板自己就写着「叫 DR1 不叫 D1」—— 两个变体共用一个名字正是记分板
#  烂掉的第一步。这处笔误早于迁档，2026-08-10 顺手订正。）

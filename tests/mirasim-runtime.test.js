...
describe('占用声明：两个并发 startSession 抢同一棵树（审官红④ / #1291）', () => {
  const freeLease = () => ({ ok: true, verdict: 'free' });
  const passChannel = () => ({ ok: true, verdict: 'ok', release() {} });

  it('第二个拿不到占用声明，一帧 prompt 都不发', async () => {
    ...
    const rtA = createRuntime({
      ...
      now: () => T0,
      leaseCheck: freeLease,
      channelAdmit: passChannel,
      claimOccupancy,
    });

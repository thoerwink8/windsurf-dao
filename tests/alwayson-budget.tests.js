...
//   ⇒ 本文件当场红 ~~**3 条**，与 issue #160 报的条数逐条吻合~~ **5 条**（`PASS=93 FAIL=5`，
//   `--env` 层实测）。**订正 2026-08-08 · PR #200 对抗官 F3**：划掉那半是**改动前**的数
//   ——「3」正是 issue #160 报的原始条数，而它被写进了**同一个把它改成 5 的 commit** 的头注里
//   （本节新增了一条「前置」断言，且可达性判据从 `/字节预算/` 换成了行首标记 ⇒ 红集本来就该变）。
//   两个数都留着才看得出这一格是怎么动的：3 = 改前 / 5 = 改后。
//

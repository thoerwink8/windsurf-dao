...
//
// 被测对象承担一个**判据**：哪些断言算「本次没跑」、「没跑」要在退出码上怎么表达。判据类东西
// 一旦自己坏掉，症状恰恰是「一切看起来正常」——defer 计数解析一失灵 ⇒ 全场 defer=0 ⇒ exit 0
// ⇒ 「没跑」又变回「跑了全过」。故这里双向断言：
//   正控 —— 该 2 的时候必须 2、该 4 的时候必须 4、该 3 的时候**一套都不许跑**
//   负控 —— 全绿零 defer 的场子不许被顶成 2；头部没标记的普通测试不许被误判
// 夹具：每例一个 `_tmp/run-tests-tier/<case>/tests/`，里面是合成的 `*.tests.js`，只打一行汇总、
// 按参数决定 DEFER 值、并往 sentinel 文件记一笔「我被跑到了」——sentinel 是「一套都没跑」那条
// 断言的唯一证据（只看退出码分不出「跑完了都过」与「压根没起跑」，这正是本机制自己要治的病）。
// ⚠ 本文件**刻意不在头部写出层级标记的字面量**：run-tests 的标记扫描面就是本目录 `*.tests.js`
// 的头部若干行，本文件正住在那个面里。标记由下面 `TIER_MARKER` 拼接而成（同 dead-gates 里
// `PH` 那一手）且位置在头部窗口之外——「检查器的输出不能落在它自己的扫描面内」同一判据换个身位。

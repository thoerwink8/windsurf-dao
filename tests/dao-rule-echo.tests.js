...
// 🔴 **射程：这四组补的是「判据被改窄（永不命中）」那一向，「8/8」只对这一向成立。**
//   反方向（判据被改宽、不该拦的也拦住）对抗官逐条量过：6 发里 ②③⑧ 三发零检出 ⇒ **实测 3/6**。
//   成因是可复用的一句：⑧ 的误伤反例 `docs/ruleset/x.md` 量的是**目录名边界**，而 `.md$` 是
//   **扩展名边界** —— **反例挑了一条轴，判据有两条轴**。补法归 issue #296。
...
// 🔴 **它证得到的只是「它自己那条路径像规则文件」，证不到下面那 9 条还在打靶**：
//   它与下面那个循环用的是**两条各自硬编码**的路径，不共享落点模板 ⇒ 只漂循环那一侧时它看不见。
//   实测（对抗官 A4）：把下面循环里落点模板的 `CLAUDE.md` 改成 `NOTES.md` ⇒ 9 条谓词 `r.out === ""` 全部恒真，
//   `exit 0 · PASS=100 · FAIL=0`，与正确态逐字节相同；两份一起漂（A9）才红 1 条。
//   ⇒ **今天这一格成立，靠的是两条字面恰好一致**。改落点时两边必须一起改。
//   修法（让两边走同一个落点函数）归 issue #295，本处只标注射程。
{

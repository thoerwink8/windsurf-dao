...
test('行注释里的 argv 赋值带 --dry-run 必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    '// argv = ["scripts/dao.mjs", "dispatch", "--dry-run"]',
...
test('块注释里的 argv 赋值带 --dry-run 必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    '/* argv = ["scripts/dao.mjs", "dispatch", "--dry-run"] */',
...
test('字符串里的 argv 赋值带 --dry-run 必须红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
...
test('行注释里的 argv.push --dry-run 必须红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    '// argv.push("--dry-run")',
...
test('真实 argv 带 --dry-run、注释里再赋无 --dry-run 仍绿', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch", "--dry-run"];',
    '// argv = ["scripts/dao.mjs", "dispatch"]',
...
test('死分支里赋 --dry-run 的 argv 必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    'if (false) argv = ["scripts/dao.mjs", "dispatch", "--dry-run"];',
    `${CALL}(process.execPath, argv, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('if (true)/if (false) 多次赋 argv 必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    'if (true) argv = ["scripts/dao.mjs", "dispatch", "--dry-run"];',
    'if (false) argv = ["scripts/dao.mjs", "dispatch", "--dry-run"];',
    `${CALL}(process.execPath, argv, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
...
test('死分支 argv.push --dry-run 必须红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    'if (false) argv.push("--dry-run");',
    `${CALL}(process.execPath, argv, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('花括号死分支里赋 --dry-run 的 argv 必须红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let argv = ["scripts/dao.mjs", "dispatch"];',
    'if (false) {',
...
test('解构默认值 { exec: run = fallback } 必须红（审官对抗样本）', () => {
  const EX = 'ex' + 'ec';
  const alias = 'ru' + 'n';
  const tick = '`';
  const src = [
    `const { ${EX}: ${alias} = fallback } = require("node:child_process");`,
    `${alias}(${tick}node scripts/dao.mjs \${getVerb()}${tick}, { env: { PATH: "/bin" } });`,
  ].join('\n');
  assert.ok(collectSpawnAliases(src).includes(alias), collectSpawnAliases(src).join(','));
  const r = classifyTestDispatchSpawns(src);
...
test('计算键解构 { ["exec"]: run } 必须红（审官对抗样本）', () => {
  const EX = 'ex' + 'ec';
  const alias = 'ru' + 'n';
  const lb = '[';
  const rb = ']';
  ...
  const src = [
    'const cp = require("node:child_process");',
    `const { ${lb}${JSON.stringify(EX)}${rb}: ${alias} } = cp;`,
    `${alias}(${tick}node scripts/dao.mjs \${getVerb()}${tick}, { env: { PATH: "/bin" } });`,
  ].join('\n');
  assert.ok(collectSpawnAliases(src).includes(alias), collectSpawnAliases(src).join(','));
  const r = classifyTestDispatchSpawns(src);
...
test('数组解构 const [run] = [cp.exec] 必须红（审官对抗样本）', () => {
  const EX = 'ex' + 'ec';
  const alias = 'ru' + 'n';
  const tick = '`';
  const src = [
...
test('计算键解构带默认值必须红', () => {
  ...
  const src = [
    'const cp = require("node:child_process");',
    `const { ${lb}${JSON.stringify(EX)}${rb}: ${alias} = fallback } = cp;`,
    `${alias}(${tick}node scripts/dao.mjs \${getVerb()}${tick}, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('动态 import default 解构赋值必须红（审官对抗样本）', () => {
  const src = [
    'let cp;',
    '({ default: cp } = await import("node:child_process"));',
    `cp.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('动态 import default 解构赋值带 --dry-run 仍绿', () => {
  const src = [
    'let cp;',
    '({ default: cp } = await import("node:child_process"));',
    `cp.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch", "--dry-run"], { env: { PATH: "/bin" } });`,
...
test('process.execPath 解构赋值 + 解析不了的 argv 必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let nodePath;',
...
test('process.execPath 解构赋值带默认值仍红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let nodePath;',
    '({ execPath: nodePath = fallback } = process);',
    `${CALL}(nodePath, makeArgv(), { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('process.execPath 字符串键解构赋值仍红', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let nodePath;',
    '({ "execPath": nodePath } = process);',
...
test('process.execPath 解构赋值但 argv 带 --dry-run 仍绿', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'let nodePath;',
    '({ execPath: nodePath } = process);',
...
test('拼接计算键对象转发必须红（审官对抗样本）', () => {
  const alias = 'ru' + 'n';
  const src = [
    'const cp = require("node:child_process");',
    `const box = { ["r" + "un"]: cp.${CALL} };`,
    `box.${alias}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  assert.ok(collectSpawnAliases(src).includes(alias), collectSpawnAliases(src).join(','));
  const r = classifyTestDispatchSpawns(src);
...
test('对象属性计算 RHS 必须红', () => {
  ...
  const src = [
    'const cp = require("node:child_process");',
    `const box = { ${alias}: cp${lb}${JSON.stringify(CALL)}${rb} };`,
    `box.${alias}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('解析不了的对象计算键转发 fail-closed', () => {
  const alias = 'ru' + 'n';
  const lb = '[';
  const rb = ']';
  const src = [
    'const cp = require("node:child_process");',
    `const box = { ${lb}getKey()${rb}: cp.${CALL} };`,
    `box.${alias}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
...
test('拼接计算键对象转发带 --dry-run 仍绿', () => {
  const alias = 'ru' + 'n';
  const src = [
    'const cp = require("node:child_process");',
    `const box = { ["r" + "un"]: cp.${CALL} };`,
    `box.${alias}(process.execPath, ["scripts/dao.mjs", "dispatch", "--dry-run"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, true, JSON.stringify(r));
  assert.equal(r.scanned, 1);
...
test('调用前 argv.push 动词必须红（审官对抗样本）', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'const argv = ["scripts/dao.mjs"];',
    'argv.push("dispatch");',
    `${CALL}(process.execPath, argv, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
...
test('调用前 argv.push 带真实 --dry-run 仍绿', () => {
  const src = [
    `const { ${CALL} } = require("node:child_process");`,
    'const argv = ["scripts/dao.mjs"];',
    'argv.push("dispatch", "--dry-run");',
    `${CALL}(process.execPath, argv, { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, true, JSON.stringify(r));
...
test('解构默认值静态非 dispatch 仍绿', () => {
  ...
  const src = [
    `const { ${EX}: ${alias} = fallback } = require("node:child_process");`,
    `${alias}("node scripts/dao.mjs worker-done", { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, true, JSON.stringify(r));
  assert.equal(r.scanned, 0, JSON.stringify(r));
...
test('审官 default 转发 / 计算属性适配 / exec 动态模板都不许 scanned:0 ok:true', () => {
  ...
  const samples = [
    ...
    [
      `const { ${EX}: ${'ru' + 'n'} = fallback } = require("node:child_process");`,
      `${'ru' + 'n'}(${tick}node scripts/dao.mjs \${getVerb()}${tick}, { env: { PATH: "/bin" } });`,
    ].join('\n'),
    [
      'const cp = require("node:child_process");',
      `const { ${lb}${JSON.stringify(EX)}${rb}: ${'ru' + 'n'} } = cp;`,
      `${'ru' + 'n'}(${tick}node scripts/dao.mjs \${getVerb()}${tick}, { env: { PATH: "/bin" } });`,
    ].join('\n'),
    [
      'const cp = require("node:child_process");',
...
test('拼接模块名 require("node:" + "child_process") 必须红（审官对抗样本）', () => {
  const src = [
    'const cp = require("node:" + "child_process");',
    `cp.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
...
test('module.require(child_process) 必须红（审官对抗样本）', () => {
  const src = [
...
test('嵌套 holder box.inner.spawnSync 必须红（审官对抗样本）', () => {
  const src = [
    'const box = { inner: require("node:child_process") };',
    `box.inner.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
  const r = classifyTestDispatchSpawns(src);
  assert.equal(r.ok, false, JSON.stringify(r));
  assert.notEqual(r.scanned, 0, JSON.stringify(r));
...
test('拼接 specifier 嵌套 holder 必须红', () => {
  const src = [
    'const box = { inner: require("node:" + "child_process") };',
    `box.inner.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch"], { env: { PATH: "/bin" } });`,
  ].join('\n');
...
test('拼接 specifier 带 --dry-run 仍绿', () => {
  const src = [
    'const cp = require("node:" + "child_process");',
    `cp.${CALL}(process.execPath, ["scripts/dao.mjs", "dispatch", "--dry-run"], { env: { PATH: "/bin" } });`,
  ].join('\n');
...
test('regex.exec 不因成员调用 fail-closed 误红', () => {
  const src = [
    'const re = /escalate\\/(.+)$/;',

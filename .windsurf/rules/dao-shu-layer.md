...
**在任意项目中编辑 dao-* 文件时，必须过三关**：

1. **通用性检验**：这个改动换到一个完全不同的项目（不同语言、不同框架、不同领域），还成立吗？
   - 成立 → 属于 dao（元层），保留
   - 不成立 → 属于项目（操作层），应写入项目的 AGENT.md 而非 dao 文件

2. **内容边界**：dao 文件只允许以下内容：
   - 思维方式（怎么分析、怎么决策）
   - 工作流程（怎么开发、怎么审查）
   - 行为准则（怎么交互、怎么推进）
   - **禁止**：具体技术选型、框架偏好、API 细节、项目配置

3. **影响评估**：这个改动会不会让其他已链接项目的 AI 行为变差？
   - 如果不确定 → 不改 dao 文件，改项目文件

**不通过 → 拒绝修改 dao 文件，将内容路由到项目的 AGENT.md 或对应项目文件。**

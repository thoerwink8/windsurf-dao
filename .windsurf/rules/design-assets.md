...
1. **侧边栏列表项**：每项用不同 badge/颜色表示不同项目状态
2. **备选状态区**：主内容下方用 `↓ 备选状态：xxx` 标注分隔，展示 loading/empty 变体
3. **内联状态差异**：同一网格中让不同卡片/选项处于不同交互态（neutral/hover/selected/dragging）
4. **浮动标注**：用 `layoutPosition: "absolute"` 的小标签标注特殊状态（如"拖拽中"）

### 不需要的

- **不为每个状态单独建屏**——状态在同一屏内共存
- **不需要穷举所有排列组合**——每种状态至少出现一次即可
- **Dark 模式通过 theme 切换**——不为状态单独做 dark 版

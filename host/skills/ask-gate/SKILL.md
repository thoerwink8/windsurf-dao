...
第二格（#965）：同一时刻再看相关 issue 的**评论**（不是正文）里有没有拍板痕迹。正文是快照，待拍板选项复制进去就会过期（#944 的 5.5/5.6：用户纠正过，读正文的人照着陈旧选项又问了一遍）。三态 `decided` / `none` / `unscanned` 与上面那组平行——没有 issue 号、网络失败都是没查成，**不许说成没拍过**。命中就把评论原文摘要推到眼前，不拦提问。

语义那部分机器不判：「这件事算不算花钱」由 AI 自己写依据交代，机器只验依据在不在四条里。跟 `dao-mode` 的 `selfie --basis` 同一个思路。

## 该问的时候，怎么问

判据管「该不该问」，这节管「问出来长什么样」。三处都要写，不是三选一：

1. **标题**：写明推荐哪一条，人扫一眼就知道荐什么。
2. **选项**：推荐项放第一项，`AskUserQuestion` 的 label 末尾写 `(Recommended)`。
3. **介绍**：用大白话讲来龙去脉——怎么走到这一问、为什么荐这条。不许只甩术语，也不许只写「我推荐第一项」。

反例：选项标了 `(Recommended)` 但介绍全是内部词；或推荐只写在聊天正文里，题干和标题都没有。

**降级成纯文字编号时同样适用**（全局约定里那条降级顺序：`AskUserQuestion` → `mcp__mirasim__im_ask_user` → 纯文字）。没有弹窗的前端只是渲染变了，推荐位一样要标——把「—— 我推荐」写在那一条选项后面，并说一句「这个前端没有弹窗，请回编号」，否则用户会以为你忘了问。

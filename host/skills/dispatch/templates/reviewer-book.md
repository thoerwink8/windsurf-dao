...
注入 `m=` 继承派工记账；带 `fb=` 表示读不到记账、已回退默认 auto（不要自行改成 manual）。
...
**`d=` 为空（没有对方 dispatch）→ 红项直接上帅，不打士兵**（skip-wait 补审官，或 worker-done 起审官时士兵 dispatch 已结算，#799）。

### 2. 判红 / 判绿

**判定怎么交到 GitHub（#573：你现在能真 approve；#807 删判定行协议）**：
你是 `dao-reviewer[bot]`，不是工人那个账号。「同账号不能批准自己 PR」已经不挡你。
机器可读落点是 GitHub review 状态本身：绿走 `--approve`，红走 `--request-changes`。
不要再写「判定：红 N 项」字符串协议——那套已删。
...
- **绿**：按注入参数的 merge-policy 收口（#511 帅只感知不做关口；#559 把机器可读落点钉在 PR 上），两条路分开：
  ...
  合并前（两条路都跑）：`node scripts/dao.mjs pr-sync-labels --pr <PR号>`——按 PR head 分支从账本
  打 `model/*` `type/*` `reviewer/*` 到 PR（#1116：选型只读 PR 自己的 label；查不到记录 ⇒ 需人工打标，
  不读 issue。非零退出 = 没打成，查报错补上再合并，不许带着空 label 合）。
...
返工完成后士兵仍调 `worker-done`（首行「返工完成」）。起审官失败（含士兵 dispatch 里不能再 worker-start 的深度限制、审官终端已有在途派单）时，`worker-done` 把待办写入 `_flow/queue/review-pending/<pr>.json`（含 head、工人树、reviewer）并**成功交卷**（`queued:true`，不是「没查成」非零退出）。指挥官 / automations 调 `dao.mjs review-pending-drain` 逐条 `reviewer-attach --skip-wait`。审官终端已有活 dispatch 时 drain/复用**跳过 worker-start**，把复审任务书送到现有终端。不要自己再派一张审官，也不要在已结算的审官 dispatch 上等复审。

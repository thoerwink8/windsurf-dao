...
5. 全部提交并推送之后跑 `node scripts/handoff-check.mjs`（交卷闸，#904）——核的是「你这棵树跟 master、跟远端对不对得上」：基底含不含最新 master、有没有反向删掉别人的文件、新写的仓内指针指不指向空气、审官看到的是不是你自证过的那份。退出码 0 才准往下；1 是真红，2 是没查成，**两者都不许交卷**。

...
3. 切状态：`orca worktree set --worktree active --workspace-status in-progress --json`。卡名由 `dispatch` 写成 `ISSUE-N 工人·模型 动宾`，`worker-done` 见到 PR 会改成 `PR-N …`——不要手改，更不要用 issue 号去对审官卡。

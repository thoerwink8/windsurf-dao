...
- 判官照选型表换厂商，不因快马降格。
- 起法不依赖 orca 卡态：正式路径 = `reviewer-create --pr <N> --executor mirasim`。`dao.mjs send` / `reviewer-attach` 已随 orca 退役，调用即拒。
...
派完即回对话态，帅不前台长等。#807 起本机信箱台 / 守卫保活已删，服务器承重面是 systemd + 指挥官（盘面推进量并进 `commander-act`）。**人用窗口（帅 / 主帅）永不当 coordinator**（#667）：裸 `run-use` / `run-create` 和心跳被派工闸拦住。`dao.mjs dispatch` 不 `run-use`。例外（#675）：工人 TUI 上 `bindStation` 在 `run-current` 为 null 时对本窗 `run-create`；**帅窗不许触发这条**。真信进 GitHub 评论与飞书 hub；不靠输入框横幅，也不挂 `check --wait`（一个 run 只允许一个 actionable waiter，再挂会 `waiter_exists` 刷屏，#525）。心跳不准发到 Run（#667）。orca orchestration inbox 已随执行体退役。循环跑外部命令的监视脚本必须让「同一条错误连续出现」收敛（计数/退避/自杀），否则一个稳定失败就是刷屏机器。
...
向用户汇报工位状态前，先实刷 mirasim 树 / 会话名单与 gh pr 状态——凭上次印象汇报会状态失真（2026-08-14 三次实测，issue #443）。
...
`dispatch --batch` 与 orca 版 `batch-book.md` 已随执行体退役（#1150）。调用当场拒。
只读判定若还要批量做，走 `dao.mjs dispatch` 的 mirasim 单卡路径，或另开单，不要再找 batch 模板。
...
**label 是校准数据源（#564 / #1116）**：`dispatch` 那一刻把 `model` + `reviewer` + `branch` + `repo` 写入账本；工人交卷 / 起审官前按仓 + PR head 分支打到 PR。审官/帅合并前跑 `node scripts/dao.mjs pr-sync-labels --pr <N>` 是同一条打标（幂等；缺完整记录 ⇒ 需人工打标，不读 issue）。没 label 的 PR 进不了战绩（calibrate 只认带 model/* 与 type/* 的已合并 PR）。
...
**终审接回流段（#888）**：PR 正文有 `## 回流` 段 = 士兵发现了可复用产物。合并时收口官/帅**当场接单**，三种落法之一并在段内回写证据（`dao-check --full` 的回流闸扫近 7 天 merged PR，孤儿段判红）：

| 落法 | 什么时候 | 段内回写 |
|---|---|---|
| 上收总仓 | 通用纯函数/判据 → `scripts/lib`；通用技能/任务书 → `host/skills`（落这里即进既有分发面：符号链接 + `onboard.mjs`，**不另造分发机制**） | `已回流：<sha>` |
| 开回流单 | 该上收但本单不便带（跨仓、要改调用方） | `回流单：#N`（label `回流`） |
| 判不回流 | 只此一处能用 | `不回流：<原因>`（留指针须配报警检查，见「写了指针就要配检查」） |

方向两条都靠这一个段：**自下而上**=子仓/单卡产物经此段上收；**自上而下**=落进 `host/` 后由既有分发面铺到各机各仓。子仓（如 ai-gateway-stack）的入口写在各自 `CLAUDE.md` 一行，不进全局页（常驻预算）。
...
**现役操作面**（orca 编排原语 `reply` / `gate-create` / `worker-release` / `worker-read` 已退役，调用即拒）：
- 补派审官：`reviewer-create --pr <N>`（主路 `review-pending-drain`）
- 交卷：`worker-done --pr <N>`
- 工人问帅：mirasim 会话正文提问（不要 `dao.mjs ask`）

边界三条：

1. 通道：士兵完工走 `worker-done`（入队 / 返工只发 comment），不做帅的手工转发；审官不自造旁路。
2. 必须上帅：① 审官质疑拍板/规格本身；② 乒乓两轮仍有红项（换人信号）；③ 归档动作——归档由帅执行，审官只发「可归档」通知。
3. 记录不减：内部返工轮数与原因照落 PR comment（点将台返工特征的数据源），闭环不变黑箱。

人工补起审官走 `dao.mjs reviewer-create --pr <N>`。`reviewer-attach` 已退役，调用即拒。正常路径是工人调 `worker-done` 入队，指挥官 `review-pending-drain` 拉取。

起审官硬闸（#679）：工人与审官不得同厂。闸在 `reviewer-create` / `worker-done` / 换人——即审官真正落地的时刻；**不在 `dispatch`**（2026-08-23 delete-all-ceremony 拍板：派工时审官还不存在，闸是查空气）。点将台打分不加这一条。GPT UI 禁令仍优先。失败停手报，不许自动换厂。已开工的审官复用不回溯。
...
产出要进 git（commit / PR）⇒ 必走编排派工（执行体 mirasim，`dao.mjs dispatch`），主会话不下场。只读不落盘的查证类 ⇒ 主会话可自己干，或会话内子代理。
...
**闭环接线（#546 追加第五件 → #559 换官方原语 → #586 审官按需起 → #1125 入队）**：`dispatch` 只起士兵。士兵完工调 `worker-done` 入队；指挥官 `review-pending-drain` 按空位起审官。工人完工后不再自动再造审官（#730：那是洞，不是自愈）。审官「可归档」是**普通告知不是结算信号**。模板在 `host/skills/dispatch/templates/`，不硬编码进代码。

多工人仍在约束载体内；给已有 PR 补审官走 `reviewer-create`，不要 `reviewer-attach`（已退役，调用即拒）：

```bash
node scripts/dao.mjs reviewer-create --pr <N>
```
...
开工只认「该发生的事有没有发生」：新 commit / 新 push / 产物文件 mtime 在动。启动返回成功不等于已开工。orca `worker-read` 已随执行体退役。
...
1. **指令**：一律直给提示词（`dao.mjs dispatch` 注入），不落文件。吞注入就停手报未送达——没有 `terminal send` 可补（orca 已退役，调用即拒）。
...
- 派工走哪条通道看一件事：产出要进 git（commit/PR）的必走 `dao.mjs dispatch`（执行体 mirasim）；只读不落盘的查证类才可用会话内子代理。

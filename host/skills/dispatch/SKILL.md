...
- **流转器**：PR comment 首行「完工」是流转器 `scripts/flow.mjs:183` 的触发器（工人完工且无审官 → 自动起审官）。工人只发 worker_done、不发这条评论，这个功能一次都不会触发（#575 ⑥）。发评论走 `node scripts/gh-as.mjs worker -- pr comment <PR号> --body-file <文件>`，格式见 worker-brief。

...
- **流转器（#575 ⑥ 订正）**：交棒发到 **issue comment** 首行「完工」（issue 一直在，不绑 push）。`scripts/flow.mjs:183` **此刻仍读 PR comment**——改读 issue 未在本单落地。工人发评论走 `node scripts/gh-as.mjs worker -- issue comment <issue号> --body-file <文件>`，格式见 worker-brief。

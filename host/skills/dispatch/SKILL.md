...
看门狗双通道：快乐路径（工人自报 worker_done）之外，事故路径的轮询侦测见 issue #442（脚本随 #452 交付，本 PR 不写文件名——指针没报警就是指向空气）。
...
1. 通道：工人完工后不收信箱，「通知士兵」由协调者机械转发（不判断纯管道）；后续可由看门狗自动化（监听 review submitted → 注入工人终端）。审官不自造旁路。

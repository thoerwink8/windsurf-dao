...
## 完工（走 `worker-done`，不要自己发 comment）

编排层 `worker_done` **不能**触发流转器。交棒走 **`dao.mjs worker-done`**（#586）：它发 issue+PR 评论，并按需起审官。不要自己 `issue comment`——漏调 `worker-done` 就会「done 了但没审官」。

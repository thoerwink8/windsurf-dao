...
9. 完工发 worker_done 后终端就不再收信（续活走新建任务注入）——**返工例外**：同一单被审官打回时，流转器用 `task-create + worker-start --task <新> --terminal <本终端>` 把返工任务书作为新 TASK 块推回本终端（#480 实测：send --to dispatch 是收件箱不是推送，闲置工人不收信）；本单合并前不得收工（见下）。
10. 长产出（盲考 / 大文档 / 多文件重构）边写边存：先建目标文件逐节追加，不最后一次性写盘——中途死亡不留整份产出给看门狗（2026-08-14 拍板，issue #442）。

## 合并前不得收工（#478 拍板）

**本单合并前不得收工**——发完 worker_done、判绿/终审前，终端保持活着待命。「后来者自己解决冲突」的前提是后来者还活着；收工关终端后被撞冲突就无人 rebase，需重起工人重建上下文。

**rebase 只做合并不改逻辑**：解冲突时两边内容都要保留，不得借机改动已审过的逻辑；若发现必须改逻辑才能解 → 停手上报帅，那是规格冲突不是合并冲突（#478 拍板追加，今天 #467/#463 两次 rebase 都靠帅临场交代，应固化）。

...
审官判红（一轮或两轮，给修复方案并送达）、工人交卷与任务书对不上、CI 红原因不明、撞死指纹 #833 没接住。
...
- **换人**：审官两轮仍红时你的活是给方案送达、推动修复（2026-09-04 拍板，不再直接报帅）；
  若判断非换人不可（工人能力不够/模型不行），报帅——换人有成本，归帅拍。
- **同一单已被唤醒三次仍没闭环**：不要再试第四次。写一条 `待拍板`，说清三次都卡在哪，交用户。
- 上面两条只数**打在 PR 当前 head 上**的红和唤醒次数：工人推了新 head，旧 review 挂在旧 commit 上，
  该 PR 回到「等审官」重新起算（判绿只对当时那个 commit 有效，判红同理）。head 或 review 的
  commit_id 没查成 ⇒ 按「没查成」走，既不清零也不当仍红。

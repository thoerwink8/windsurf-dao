...
| D | ~/.codex/.tmp | codex 每次 git 操作留下的临时工作目录，只增不减。2026-09-10 实测攒到 11109 个（当天只占 65 个），拖慢同盘一切文件系统遍历。board-gc `--apply` 调 `planOrphanGc` 按时效回收；进程占用没查成或目录正在用则保留。运行态，不进 git |

...
| D | ~/.dao/board-stuck.json | 盘面卡况，值守提问闸的主判据（#1287）。写：指挥官 `cmdAct` 每轮覆盖（`writeBoardStuck`），字段只有 `at` / `stalledRounds` / `waitingUser`。读：dao-mode 的 UserPromptSubmit hook（`read-board.mjs`），超 90 分钟未更新即当没查成、退回时长兜底。运行态，换机不拷 |

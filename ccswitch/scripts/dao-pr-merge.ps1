...
.DESCRIPTION
    ...
    **三个不由脚本兜住的边界（照直写，别当它全包）** —— dao.md Shell 节只留一句指针，
    **这里是这三条的正文与唯一真相源**（成因见 rationales §Shell-1）：
      ...
      ③ `--delete-branch` 的**沉默不可信**：本地分支被 worktree 占用时 gh 整体失败，而错误
         信息**只提本地** ⇒ 远程分支也没删，你却以为只是本地没删。故本脚本先拆 worktree、
         末尾再独立实查一次远程 —— **别把它的静默读成干净**。
...
.NOTES
    ...
    判据正文：windsurf-dao/ccswitch/dao.md · Shell 独有项「PR 合并期的机械链走脚本」
              ...
              ⚠ 其中**三个不由脚本兜住的边界**与**patch-id 三判据**的正文已于 2026-08-02

...
//    **2026-08-07：那个推论被拍板执行了**（issue #122 件②）—— `ccswitch/agents/` 下新增四份
//    官种型 profile（`dao-implementer` / `dao-adversary` / `dao-scout` / `dao-dogfood`），
//    帅侧条款 `[#派-官种底座]` 要求按官种选 agent type。**本表一个字没动**（判据同上：
//    问题不在表里）。⚠ **上面那组分布数字是 2026-08-02 的快照，本批没有重算**——
//    它衡量的正是件② 要改变的那个量，下一次重算才说得出这条修法有没有生效。
//    接线侧的回归网在 tests/subagent-clauses.tests.js §⑦′（profile 名 ↔ 映射表那条缝）。
//
...
//
// 🔴 **上面这格 2026-08-07 换了内容，原文照录以便对账**：原写「仍然没证到的那一半 ——
// 渲染得出东西 ≠ 这个 hook 被调用过 …… 现在仍然没有人有资格说」。**那句话已经过期**：
// 同日两名官第一人称实证，开场即收到带签名的注入，末行
// `CLAUSE_RENDER_SUMMARY exit=0 role=general general=70 role_clauses=0` ⇒ 注册与调用都坐实。
// （判据仍在 --selfcheck 第②段：它只采信非 synthetic 的心跳记录。
//  「没注册」与「注册了没触发」在日志上长得一样，所以这个结论只能由**收到注入的那个官**说。）
// 🔴 **换上来的那一半**：两次的 `role` 都是 `general`、官种节 **0 条** —— 通道活着，
// 官种筛选空转。这不是本文件的缺陷，是 `agent_type` 上没有官种信息（见上「摸了全域分布」段），
// 修法在派单侧（issue #122 件②，2026-08-07 落地）。**它有没有生效本批未测**：
// 判据是下一批派单的注入里 `role=` 还是不是 `general`。
//

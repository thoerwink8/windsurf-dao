...
// 机器投影，它每回合必到、不需要谁想起来。
//
// **本批刻意不动 dao.md 正文**（重写批 3 才删原段）。所以文字自检与本 hook 并存：
// 两者同时命中时后果只是「AI 已经想同步了，又被拦了一次」，无害；反过来 hook 没注册时
// 文字仍是唯一兜底 —— 这正是「先立后破」的形态。
// ⚠️ **此处原写「所以现在是双通道并存」，2026-08-02 起不成立**（当日加了第三条通道，
// 见下一节）。留着这四个字会让下一个读者在同一份头注里读到两个互相矛盾的通道数 ——
// 而「同一条规则三份措辞、各自漂移」正是下一节要治的病，头注本身先犯一次就没法自圆。
//

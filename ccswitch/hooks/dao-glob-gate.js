...
//   - **被 mutation 守护的源文件 → 提醒先读写守卫判据**(见下)
//
// 配在 PostToolUse(改完即提醒,语义比 PreToolUse 更贴"收尾前验证")。
// 始终 exit 0,只注入不阻断。
//
// ── 守卫指针分支 ───────────────────────────────────────────────────────────
// 「改守卫前先读写守卫判据」的读触发通道(ccswitch/rules/scoped/ 的 paths: glob)对
// 极少被 Read 的文件结构性失明(实测当事文件 11 天 Read 0 次 / Edit·Write 7 次)⇒
// 本分支挂 Edit/Write，判据清单由 ccswitch/lib/guarded-scan.js 从 mutation 测试实况
// **运行时现算**(带指纹缓存；无派生物——手维护必滞后，派生物必漂移)。
//
// 三条设计约束,都是刻意的:
...
//      凡是**部署副本可能滞后于仓内版本**的时刻都成立(~/.claude/ 的链接指向别处、别的 worktree、
//      别的机器还没拉):一份不认识 `--selfcheck` 的旧副本会**静默 exit 0、零输出**,与「自检通过」
//      在退出码这个唯一的机器通道上**逐字节相同** —— 「数到 0 和没看到样本,输出一模一样」。
...
// 回归网：tests/glob-gate.tests.js(前四条分支的正控 + 误伤负控 + settings.json 文案 + 判别力 mutation)。
// 守卫指针分支与 guarded-scan 的口径见 ccswitch/lib/guarded-scan.js 头注（该分支的回归网随
// 重设计退役，勾选它的是 --selfcheck 与 glob-gate 测试的剩余断言）。
...
if (isDaoMeta) {
  ...
  // ⚠ **快照层 config-sync/common/settings.json 与 DB 的 common_config_* 镜像层都不在下发路径上**——
  // 照旧文案「改快照 + 跑 dao.bat --direction=down」做改动永不生效(实测)。**一道提醒给出走不通的合法路径,
  // 比不给更糟**:照做的人会以为自己已经做完了。故措辞点名 `dao.bat --direction=down/up` 但把它从「正路」
  // 改成「别拿它来让配置生效」(旧说法散在历史文档里,光删不说等于让下一个人再试一次)。
  ...
  // **不要建议跑 `--direction=up`**:up 走 config-sync/lib/export.mjs 的 `selectRows('settings',
  // "WHERE key LIKE 'common_config_%'")` —— 只从 SQLite DB 读,**看不见 ccswitch/ 下的 git 源文件**;
  // 对 ccswitch/ 的改动,up 不仅无必要,还会用旧 DB 状态覆盖快照。该错误提醒曾连续误导多名 worker
  // 照抄进交付报告,属「每次都在误导」形态,优先级高于一次性错误。

...
//       ⇒ **别把这 4 格当成「还没来得及修」再修一次。** 理由 / 解冻条件 / owner 见台账
//       `docs/ops/nogo-ledger.json` 的 [NOGO:g2-rename-4gaps]（本行是台账落点指针；标记与台账的
//       机检已随 PR #307 退役，现状见台账 `_doc`；LEDGER_117 钉的是行为值，不是这个决定本身）。
...
//       ✅ **已修，分两批（2026-08-09 · PR #235 建闸 + issue #254 校准估价系数；系数 =2.5×
//       由用户当日拍板，出处 issue #70 评论 5231799900）** —— 上一行那句光秃秃的「已修」当时
//       被同代对抗官实测证伪过（`#官通-禁笃定措辞`），订正经过归档案层，见本格末尾。
//       **病灶：相③「重跑 judge」那条路是个单 token 就能打开的放大器。** 首版没有开销上界：
//       攻击者只需再塞一个「展开后恰是 live 目录本身」的候选（末位放，落进 `batch.untried`）
//       即可强制 `g2Phases` 把整条判定重跑一遍。实测（N=48000 诱饵）：不带这个 token 5.3 s、
//       带上 11.4 s ⇒ 越过宿主给这个 hook 的 10 s ⇒ **整个 hook 被杀，七道闸一起放行**——
//       包括那条本该被饿死 fail-close 拦下的命令。
//       **修法**：给这条重跑路自己的开销上界（`G2_RERUN_DEADLINE_MS`），用**这一次相① 的真实
//       耗时 × `G2_RERUN_COST_FACTOR`（=2.5）**估价"重跑一遍要多贵"，预算不够就不硬跑，退回
//       饿死 fail-close（诚实说"没验完"，不静默放行、不起任何进程）。完整论证、常量与判据都在
//       `g2Phases` 正文（㈡ 那段 + 预算闸三常量），不在这里重复。
//       **系数为什么是 2.5 而不是 1**：12 组样本实测 v3/v1 = 1.88–2.64（均值 ≈2.22），首版判据
//       等于把这个比值当"约等于 1"，于是在一段 N 上闸判"还来得及"其实来不及（本机实测
//       N≈40000–52000 这一段仍会越宿主 10 s）。**窗口用 v1 表述、不锚绝对毫秒**（换机器仍成立）：
//       本机 `elapsed ≈ 1.31·v1` 的拟合下，危险窗 ≈ v1 ∈ (2.8s, 3.5s)——攻击者调 N 就能把 v1
//       落进这个区间，N 是攻击者挑的。系数落地后同一批样本回测：N=40000/44000/48000（原危险窗
//       内）全部拦下、N=32000 仍放行（总墙钟 7.6 s，在预算内）——取值不是凭空定的，是回测过的
//       那个数。
//       **回归网**：小规模 + 界 1 ms 的判别力断言（不依赖机器速度，钉住"预算闸确实先于饿死
//       阈值生效"，PR #235 遗留）+ 危险窗内真能量到预算闸的判别力断言（issue #254 ①，替掉了
//       此前那条实测走末尾饿死闸的空过断言；系数改回 1 即由 2 翻 0）+ 真实规模（N=48000）软
//       证据（`#官通-性能哨兵` ①，不锚绝对毫秒）。
//       **未尽处照直写**：2.5 是对那 12 组样本的回测结果，不是对 v3/v1 分布的穷尽证明——比值
//       的理论上界未知，`G2_RERUN_SAFETY_MARGIN_MS`（2000ms）兜的是这批样本之外的残余低估，
//       边际够不够只由已回测的这 12 组 + 危险窗判别力断言背书，换一批更极端的语料仍可能露出
//       新缺口（与 ⑳ 同源：照做档的"数值取够不够"永远只到"已验证过的样本"为止）。
//       **订正经过归档案层**（`#官通-划线保留` 的代码侧射程：代码注释里的过期结论直接改真、
//       不留划线，谁在第几轮把它证伪的归 issue）：「已修」→ 被证伪成「部分缓解」→ 系数落地，
//       出处依次是 issue #214 → PR #235 对抗判词 → issue #254 → PR #261 对抗判词。
...
function g2Phases(judge, blocked) {
  ...
  // **实测 v3/v1 = 1.88–2.64（12 组样本，均值 ≈2.22）——不是"略高"，是约 2.2 倍**，故这个
  // 估价要乘 `G2_RERUN_COST_FACTOR`（=2.5，用户 2026-08-09 拍板，issue #254）再拿去比，判据
  // 见下方 `elapsed + G2_RERUN_COST_FACTOR * v1CostMs > G2_RERUN_DEADLINE_MS` 那一行；
  // `G2_RERUN_SAFETY_MARGIN_MS` 从此只兜系数没覆盖到的残余低估与固定开销，不再独扛这一截。
  ...
  //       **这条检查给 v3 的重跑设了预算上界，而上界成不成立取决于估价准不准**：估价 =
  //       `v1CostMs × G2_RERUN_COST_FACTOR`（=2.5，用户 2026-08-09 拍板，issue #254）。系数
  //       =1 的首版把 v3 代价低估约 2.2 倍，故在一段 N 上（本机 N≈40000–52000）闸判"还来得及"、
  //       实际仍越宿主 10 s 界，**那一段这条检查不构成总墙钟的硬上界**，只是把被杀的窗口推远；
  //       系数落地后，该危险窗内的 12 组回测样本（N=40000/44000/48000）均已正确拦下。**射程
  //       照直写**：证到的是「已回测的这批样本」，不是「v3/v1 这个比值再无更大离群值」——见
  //       ㉑ 头注「未尽处」段；②解不开不静默退化——走的是既有的 `g2Starved`；

...
//     ㉑ ✅ **已修（2026-08-09 · PR #235）** —— **相③「重跑 judge」那条路是个单 token 就能打开
...
//       不锚绝对毫秒）。**账仍在 issue #214**（与 ⑱⑳ 同一张集账单）。owner：AI。
...
function g2Phases(judge, blocked) {
  ...
  // 「一律抛」多做 Map 查找 + `g2ShortExpand` 调用，实测代价略高于 v1，故留了安全边际
  // （`G2_RERUN_SAFETY_MARGIN_MS`）兜这一截低估。
  ...
  //       不让 v3 把总时长推过宿主界；②解不开不静默退化——走的是既有的 `g2Starved`；

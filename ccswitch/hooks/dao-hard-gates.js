...
//       **账仍在 issue #214**（与 ⑱⑳ 同一张集账单）。owner：AI。
...
const G2_HOST_TIMEOUT_MS = 10000;    // 宿主对本 hook 的注册超时（config-sync 快照里
                                      ...
                                      // 值不会同步到这里，取不准的方向刻意保守。
const G2_RERUN_SAFETY_MARGIN_MS = 2000;  // 留给「v1CostMs 低估 v3 真实代价的那一截」+ signal
                                          // 投递 / stderr 写出 / 进程退出的开销。判断档，AI 自定，
                                          // 已在 PR body 列为可退回项——嫌紧或松都只改这一个数。
...
function g2Phases(judge, blocked) {
  ...
  //       `elapsed + v1CostMs > G2_RERUN_DEADLINE_MS` 那一行）**：用**这一次真实测得的相①
  //       耗时**当"重跑一遍要多贵"的估价——相③ 的重跑与相① 做的是同一件事（把全部 segment
  //       重新走一遍 `g2WriteTargets`），量级天然接近，且随机器速度/payload 大小自动跟着变，
  //       不需要为换机再调一次常量。预算不够就不硬跑，退回**饿死 fail-close**（诚实说"没验
  //       完"，不是"查过没事"），不静默放行、不起任何进程：①墙钟有界——~~这条检查本身就是为了
  //       不让 v3 把总时长推过宿主界~~
  ...
  if (batch.untried.length) {
    ...
    if (dirForm) {
      // ㉑ **预算闸**：重跑一遍 judge 的代价与相① 同量级（`v1CostMs`），预算不够就不硬跑——
      // 退回饿死 fail-close（诚实说"没验完"），不是静默放行、也不是不管三七二十一硬跑到
      // 宿主把整个 hook 杀掉（那样连这一格 fail-close 本身都没机会说出口）。
      const elapsed = Date.now() - t0;
      if (elapsed + v1CostMs > G2_RERUN_DEADLINE_MS) return g2Starved(batch.fed, starved);
      const v3 = judge((p) => {
        const v = batch.map.get(p);

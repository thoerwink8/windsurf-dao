...
//      （ccswitch/lib/settings-drift.js；「结构面 + dao 归属过滤」——旧版整文件 hash 比较因
//      快照是 DB 导出格式而必然假阳性，已移除，见该文件头注）
//
// B) **所有 git 项目，含元仓库自己**：共性 rule 备案清单逐条求值
//    （另含上面最后一项，从任意项目都能查）
//    ⚠ 元仓库不再整体豁免：检查若从不跑到立法者头上，没人会发现它自己违规。
//    实测它自身会中两条，其中「根目录无冗余 AI 入口」有个从未写下来的例外
//    （AGENT_GUIDE.md 系刻意保留，dao.md 引用它）——例外逐条写进清单的 `exempt` 字段。
...
//    - product-type 条目（PR 真机证据三态 …）只对在 CLAUDE.md 里**自我声明**为「产品型项目」
//      的仓库查——对内部工具仓不合理
...
//
// 检查项为什么在 JSON 不在代码里：加共性 rule 要改代码 ⇒ 实际没人加；清单化后加共性项 =
// 往 JSON 加一条对象，触发时机焊在 SessionStart 上，不依赖任何人记得。
//
...
// 逐字节相同** 。而「哪一句会抛」穷举不完——「今天找不到能让它抛的入参」的射程
// 只到当天那组入参。
...
//   **这一行此前零断言覆盖，而它恰恰是承重的** —— 实测「写完之后才崩」两态：守卫在 ⇒
//   stdout 一段合法 JSON；摘掉守卫 ⇒ **两段拼在一起、非法 JSON**。
//   回归网见 tests/dao-scaffold-check.tests.js「双写守卫」那一组。
//
// 🔴 **`stdout.write` 自己抛出时，这张网会长出一条更静默的路**：
...
//   `claude -p` 观察宿主拿这个退出码做什么，未做）。本条不依赖它成立的那一半是：
//   **这条路上本来就不是 0**，这里只是把它退回去。
let emitted = false;
...
// （「加载失败必须响，不许静默吞」），但**代价比那两处高一个数量级**， 的对抗验证
...
// —— 那是一句**方向性安全断言**，而 PR 二轮对抗当场构造出反例，**且不用等将来**：
//
...
//
// 副本把「注册是 10 秒」这个**外部事实**焊死在了代码里 —— 那正是那个病本身的形态
...
// 🔴 **「注册 < 10 s」不是一个假想的将来态，本机此刻就有实例**（复测）：
//    复测）：同一份 `~/.claude/settings.json` 里 17 个 hook 注册中，
//   **5 个的 timeout 那一栏写的是 5 秒**（本条入库时 PR 对抗官数到的是 3 个，
...
// （订正）：原文写的是「走到这条路的前提就是那份 I/O 与解析的
...
// 原样印进报文，读者看得见、复核得了。剩余风险与解冻条件见 git 历史。
//
...
// 🔴 **这两句顶层调用必须与上面的 require 同受保护**。
// 上面那道 try/catch 是 PR 的阻断 1 修的，但它**只包住了 `require`**：紧跟着的
...
//
// **它存在的唯一理由是那条 catch 分支端到端造不出来**。PR 未尽处 1 写着「余量要恰好
...
// ── 条款库结构闸的挂载点────────────────────────────────────────
// 「规则集只增不减」那条自带 `触发:verify-all/check-clauses-structure`（ 起正文迁
...
//   · ** 起筛选判据不再是本文件自己的正则**，改为直连 clause-parser 的遮罩判据
//     （见下方 loadClauseParser 那段）。于是「独立于闸的第二套实现」这句
...
// + cc-switch DB 的 `providers` 表（后来加的第三层，**那才是真正的下发源**），
...
function budgetSummaryLines() {
  ...
  if (overBudget.length) {
    ...
    //
    // 🔴 **`⚠ ` 这个前缀是 2026-08-10 补的**（issue #273 G4，用户拍板「算发现，按同规修」）。
    // 在此之前，同一次返回里的三条尾注**两条带 ⚠、唯独这条不带** ⇒ 整行以 ⓘ 开头、不含 ⚠、
    // 也不命中任何 NON_PASS_PATTERN ⇒ `isGreenSyncLine()` 判它真绿，J2 聚合把它连同
    // 「N 行全绿」一起吞掉。而它**不是偶发**：注册 timeout=10 s ⇒ 有效截止线 8500 ms，上面
    // 那五个内层常量全部大于它 ⇒ 每次会话都出现（PR #271 对抗验证 5237938692 发⑫ 真跑
    // hook 实测：报文里原样在，喂真判据得 `isGreenSyncLine=true`，测试侧当时零覆盖）。
    // **载体为什么选 ⚠ 而不是加第 8 条 NON_PASS_PATTERN**：旁边那两条同族尾注走的就是 ⚠
    // （判据①是通用锚，新形态天然盖得住）；模式档（⑦ 那种）留给「不该带 ⚠、却也不算绿」
    // 的形态。同族同载体，下一个人加第四条尾注时才不必重新判一次。
    // **代价照直写**：只要那几个常量还够不着，这一行每次都带 ⚠ ⇒ **元仓库 daoSync 段不再
    // 聚合**。那不是副作用，是这条尾注的语义——它报的是一件真没修的事（要么把注册 timeout
    // 抬到 ≥ 最大内层常量 + 收尾余量，要么把常量降到有效截止线以内）；修掉之后 `overBudget`
    // 为空、这一行自己就没了，聚合当场恢复（回归网里那条误伤反例钉着这一格）。
...
// （，PR 对抗验证评论 挂账㈢ 实测坐实）：`writeHeartbeat()`
...
//  那次是**手动**把两个 provider 对齐的：新 provider 加入、或某个 provider
// 被单独改一次，漂移必然复发，而**当时没有任何机制会发现它**。
//
// **「整体覆盖」对 `settings_config` 里的每一个键都成立，不是 hooks 专有**：
...
// 判据两层（第二层是  · PR 对抗验证 5230986835 F1 返修，帅裁定「这一格
// 按行为修，不只改文字」）：
...
//      口径（PR 对抗验证 5231324695 订正）：「有 6 种 ⓘ 行不是语义上的绿，其中
...
const NON_PASS_PATTERNS = [
  ...
  //   dead/ambiguous 两个姊妹子项也钉住，~~理由同③~~ **PR #271 对抗验证 5237938692
  //   C2/C2b 改真：⑤ 与 ③ 不是同一种结构**——③ 锚在两个子项**共用的返回前缀**上，
  //   长出新姊妹时天然盖住（C2b 实测 probe 判非绿）；⑤ 是**逐项枚举**三个子项的措辞，
  //   `memoryRefLines()` 长出第 4 个姊妹时全套零红、那一行被判绿并被聚合吞掉
  //   （C2 实测 PASS=298 FAIL=0，probe 判绿）。「理由同③」对**已知的这三个**为真、
  //   对**结构**为假；修法（改锚共用前缀，或加基数断言）归 issue #273 G1）
...
// **从未跑过**：同步漂移、live↔快照自检、以及  才挂上的条款库结构闸，全部静默跳过。
...
// PR 对抗验证 5231324695 R3 同批订正：改一条陈述时 Grep 面要含该对象自己的源文件与

...
4. **基线位置**：截图基线文件存放路径
5. **运行命令**：一键执行全量对比的命令
6. **验证脚本**：项目必须维护可执行的 fidelity 验证脚本（见下）

### 验证脚本产出规定

每个有 `design/` 目录的项目**必须**在约定位置维护可执行的 fidelity 验证脚本。这不是可选项——没有脚本的项目，L3~L5 验证只能靠人工，不可复现也不可 CI。

| 项 | 规定 |
|----|------|
| 存放目录 | `tests/fidelity/`（与单元测试、E2E 测试同级） |
| 数据来源 | 页面-状态清单从 `.claude/rules/design-fidelity.md` 状态矩阵提取 |
| 覆盖要求 | 状态矩阵中的**全部态**，不只是默认态 |
| 输出路径 | 审计截图 → `_tmp/qa/fidelity/`；回归基线 → `*-snapshots/` |
| 命名规范 | `<type>-<page>-<state>.png`（type: `audit|compare|verify`） |
| CI 集成 | 全量脚本纳入 CI，PR 合入前必须绿 |

**参考模板**：本 skill 的 `templates/` 目录提供按技术栈分类的参考实现。项目首次落地时，AI 从对应模板派生具体脚本，填入项目的页面清单和状态矩阵。模板按步骤规范（§6.5）构建，项目可加步骤不可减。
...
**L2 结构层的自动化补充**。状态矩阵（§6.4）解决"有没有覆盖所有态"，布局完整性解决"每个态的窗口是否被内容填满"。

**问题定义**：CSS Grid/Flex 容器若缺少行/列模板（如 `grid-template-rows`），子元素可能无法填满容器，在窗口边缘留下可见的死区（dead zone）。这类 bug 的特征是：

- Token 全对、像素 diff 可能通过（死区在 diff 图边缘不显眼）
- 仅在特定数据状态下暴露（内容短时死区更大）
- 人工截图不易发现（需要对比窗口底部/右侧边缘）

**检测方法**：取最外层布局容器（如 `[data-slot="app-shell"]`）的最后一个可见子元素的 `bottom`（或 `right`），与容器自身的 `bottom`（或 `right`）做差值。差值超过阈值（默认 20px）→ fail。

**适用范围**：**所有页面、所有状态**——不分有无设计原型。这是结构层断言，不是视觉层检查。

**豁免**：模态对话框（Dialog/Modal）覆盖整个 shell 时，对话框遮罩本身就是最外层可见元素，不受此检查约束。

**项目落地**：
- 项目在 `tests/fidelity/mock-helpers.*` 或等效文件中实现 `assertNoLayoutGap` 工具函数
- fidelity 验证脚本的每个截图步骤后调用此函数
- 阈值可在项目 `.claude/rules/design-fidelity.md` 中覆盖（默认 20px）
- 容器选择器可配置（默认查找 `[data-slot="app-shell"]` 或 `body > *:first-child`）

**为什么放在 L2 而不是 L3**：像素 diff（L3）比较的是"看起来像不像"，布局完整性检查的是"结构填没填满"——不需要基线，不需要设计原型，纯粹是 DOM 几何断言。属于结构布局（L2）的自动化延伸。
...
4. **布局完整性检查**（§6.4.1）：**所有页面均执行**，检测窗口边缘死区。模态对话框页面豁免
5. **有原型 → 像素 diff**：与设计原型基线对比，超阈值则 fail
6. **无原型（代码独有态）→ 空白检测**：内容区面积占比 > 30%，否则判为大面积空白 fail
7. **设计原型截图**：由自动化工具启停 `design/` HTTP server 截图，存为基线（`*-snapshots/`，tracked），不存 `_tmp/`

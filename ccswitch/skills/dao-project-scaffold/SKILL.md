...
- **内部工具 / 基建 / 一次性脚本仓** ⇒ **把那一行整行换成负向声明**「本仓是**内部工具型项目**。」（骨架里的说明注释照旧删掉）。那一档不查。

> **2026-08-02 变更（负向声明改了写法，旧文档若写「删掉那一行」以本节为准）**：负向此前的处方就是删掉那一行，
> 于是在机器侧「答了，不是」与「从没答过」**逐字节相同** ⇒ 这道题**永远送不到存量项目面前**
> （要提醒谁？所有没写那五个字的仓里，绝大多数已经答过「不是」了）。两个答案各有各的串之后才分得开，
> 清单条目 `product-type-answered`（universal · info）据此把这道题投递给**两串都没有**的项目。
> 🔴 **负向必须原样抄那六个字**：判据是纯子串，而「不是…」「非…」这类自然的否定写法里
> **整段含着正向那五个字** ⇒ 会被读成「是」。这一向没有检测器（全生态实测零实例，不为假想敌立判据），
> 只有骨架与报文里的文字警告。
...
**照直写它仍然做不到的那一半**（2026-08-02 更新：原文写「手工建 `CLAUDE.md` 的路径它管不到……没有任何东西会响」，那半已由 `product-type-answered` 补上——手工建的 `CLAUDE.md` 只要两串都没有，每次 SessionStart 都会响一行）。**现在还剩的三个洞**：①**否定句写法**（「不是产品型项目」）被判成「是」，无检测器；②把这两串写在正文别处（例如引用那条报文）的项目会被判「已答」，判据是纯子串、认不出上下文；③本条只保证问题**被送到眼前**，不保证有人答——它是 info 级、不阻塞，与「答没答」之间仍隔着一个人的动作。
...
③ **答必答题**——若本轮物化了 `CLAUDE.md`，**立刻**按 §项目类型必答题问出答案并落笔（产品型 ⇒ 留声明行；内部工具 ⇒ **把那一行换成负向声明**，不是删掉——判据与理由见该节）。骨架里其余占位（项目名 / 这是什么 / 铁律 / 验证入口）一并填掉，别留一份全是尖括号的文件。

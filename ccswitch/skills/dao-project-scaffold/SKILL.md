...
若检测到前端信号（`package.json` 依赖含 `react` / `vue` / `svelte`，或存在 `src-ui/` / `apps/desktop/` 等前端目录）：
...
**若检测到前端信号（`package.json` 依赖含 `react`/`vue`/`svelte`，或存在 `src-ui/`/`apps/desktop/` 等前端目录），额外检查：**

- [ ] `.claude/rules/frontend-style.md` 存在（声明样式技术路线，从 `stacks/` 对应处方派生，见上方「前端技术栈门控检测」）

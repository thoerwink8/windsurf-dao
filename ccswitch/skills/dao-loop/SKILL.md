...
5. 验证 git 工作区干净度
6. **命令同步检查**：验证 `~/.claude/commands/dao-loop.md` 存在且非 0 字节。缺失或空文件 → 提示运行 `pwsh -File <windsurf-dao>/dao.ps1 link-claude`（无 pwsh 时回退 `powershell -File`；fortify2-20260726 D6：sync-commands.ps1 已删除，覆盖面更窄且被 link-claude 完全取代）

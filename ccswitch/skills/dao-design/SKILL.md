...
**提示稿挂起巡检（消费队列，扫描时顺带执行）**：活跃草稿区中状态含 🟡「提示稿已出，待 OD 出原型」
的行，若登记日期距今 **> 7 天**仍未消费（无对应 workspace 原型产出），本次回答中浮出一行提醒并给
用户三选一：现在去 OD 出原型（自动跑 od-sync 备好面板）/ 降级为直接代码实施（提示稿转 HANDOFF）/
作废清账（CONTEXT 划线注明）。不阻塞主流程，只提醒——提示稿悬空无人消费是实证过的断链点。
...
> AskUserQuestion 硬限 2-4 个选项。每个模式的选项 **≤ 4 个**。未占槽位的低频功能通过**提问文本**暴露关键词（用户选 Other 输入后走 §P.1 意图识别路由）。
>
> 提问文本模板：`请选择操作（也可输入：{未占槽位的功能关键词}）：`
...
7. **选项超 4 个** — AskUserQuestion 硬限 2-4。低频功能在提问文本里暴露关键词（走 Other → §P.1），不占选项槽位

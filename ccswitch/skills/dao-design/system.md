...
| **布局** | — | 三种策略 Cap+Center / Stretch / Multi-column，详见 `standards.md` §L |
...
**用户入口只有 2 个**：`/dao-design system`（低频）+ `/dao-design open`（高频）。fidelity / radar / qa 由 design-open auto-gate 自动触发。判据来源统一引用 `standards.md` §4。

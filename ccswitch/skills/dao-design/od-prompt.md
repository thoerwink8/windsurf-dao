...
完成后在同目录创建 WORKSPACE.md（字段与格式见 `asset.md` §O.1 唯一 canonical 模板，`source` 固定填 `design`，`page`/`scope` 按本次任务填写）。
```
...
**HANDOFF.md 六节结构**：以 `asset.md` §O.2 为唯一 canonical 定义（CLI 侧 §C.2 的实施依据锚点），本节不重复列举，缺一不可。

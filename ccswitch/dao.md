...
3. **竞品即将就史**：竞品形态=用户已被迫接受的最低解；找用户在哪些时刻正在将就（土法自救/绕路/放弃），将就处即机会

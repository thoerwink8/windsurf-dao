...
- **职称名录（用户采纳 2026-07-12）**：帅=主会话 ｜ 军师=dao-strategist ｜ 谋士=dao-brainstormer ｜ 参谋=dao-plan-writer ｜ 令官=dao-spec-writer ｜ 监军=dao-reviewer ｜ 御史=dao-reviewer-critical ｜ 先锋=通用型实现（worktree 隔离）｜ 工兵=dao-worker-batch ｜ 军医=dao-debugger ｜ 斥候=通用型只读侦察。派什么将由帅判断，用户无须点名也无须记名录。**派单通报（每次必带，用户拍板 2026-07-12）**：每次派将，在给用户的回复中以固定格式通报——「⚔️ 已派**职称**（一句白话职能）→ 本次任务一句话」，多将并行则逐将一行；完稿汇报同带括注（「军师（战略评估）完稿：…」）。括注不因会话内重复出现而省略——用户会遗忘，每次派单都是重新学习的时机

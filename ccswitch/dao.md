...
- **长窗自主排程**（用户拍板 2026-07-17：无明确目的的长窗，帅自主判断现状定任务，直到满足时长）：
  ...
  ②.5**外向汲取（常态四源 + 见底兜底）**（用户拍板 2026-07-22 立、2026-07-27 扩为常态）：**外部输入是常态不是偶发**——用户原话「不断汲取网上最佳实践和 e2e 发现卡点和竞品优秀的地方抄袭和竞品评论区 or 社媒用户真实痛点」。**四源**：①**业界最佳实践**（做技术/交互决策前先查业界怎么做，不闭门发明）②**e2e 卡点**（自己端到端跑一遍，记「卡在哪」而不只记通过/失败）③**竞品优秀处**（学形态与信息架构，**clean-room 红线内**：只看界面/文档/issues/releases，禁读其源码）④**竞品评论区与社媒真实痛点**（GitHub issues / HN / Reddit / 社媒 / 应用评论区）。**常态化靠挂在必经动作上**（按遵守率实证，无标记时刻的自由裁量必失效）：用户面功能批**派单契约门**须附「外部参照三问」——竞品同面形态 / 业界最佳实践 / 用户真实痛点，各一句话+出处，查了没有就显式注「无先例」，**不许留空**；**dogfood 收尾**须记卡点清单；**见底前**再兜底扫一遍未消费源。入池一律过分层读（bug 近字面采信 / 功能请求处方可疑痛苦真实）+ 场景归因。**⚠ 四源皆「有声的加法源」，须配两个反向源**（2026-07-27 帅补，用户认可）：**⑤减法自问**——四源只说该加什么，与「为道日损」相冲，功能越多越难打磨；每批须问「这批之后有什么该删/该合并/该默认关掉」；**⑥无声失败扫描**——无障碍/边界数据量/极端环境的受害者不发声直接走人，竞品也常同样忽略、照抄照不到；且 e2e 由熟练者跑，**跑不出新手会卡的地方**（结构性盲区，近似解：专看竞品新用户抱怨 + 刻意以「第一次见」走首启链路）。豁免：无同类竞品 / 病期救火窗 / 短窗（<4h）。细则见 `docs/playbooks/outward-loop.md`。

...
**写入位置 ≠ 归属地？自动触发。** 每次产出文件时，过这棵树：

```
产出了文件？
  └─ 写在了哪里？
       ├─ 归属项目目录内 → 已归位，无需提醒
       └─ 用户目录（~/.claude/ 等）或临时位置 → ⚠️
            ├─ 内容属于某个 git 项目（如 windsurf-dao）？→ 必须归位提醒
            └─ 纯本机个人偏好、无项目归属？→ 不提醒
```

关键信号：文件**写入位置**与**归属项目**分离——这是最容易遗漏归位的场景（如在 `~/.claude/hooks/` 创建了属于 windsurf-dao 的 hook）。
...
当下一步涉及某个 skill/command 的 **🔒 必止** 门控时，续力选项必须**指向门控本身**，不得跨过门控直指终态。

| ❌ 跨过门控（禁止） | ✅ 指向门控（正确） |
|-------------------|------------------|
| "开新 Loop" | "展示 Loop 计划" |
| "执行 superpowers" | "准备 superpowers 方案" |
| "开始造线" | "确认 Go 检查点" |

**原理**：选项被选中后，AI 天然把「用户选了」等同于「用户确认了」——"开新 Loop"被选中 → AI 跳过计划展示直接开工。将措辞锚定在门控步骤上（"展示计划"而非"开 Loop"），从选项生成层面消除这种错位，比事后加警告有效得多。

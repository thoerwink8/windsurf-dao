...
- **人·验**（动之后）：察回响 · 验终态 · 异常即止 · 完成流水线（顺序强制，不可跳步）：**① 构建/测试通过 → ② 设计同步门控 → ③ 声明完成**（含提交问询 / AskUserQuestion / 结果报告——任何向用户表示"做完了"的信号）。② 未完成时禁止进入 ③
  ...
  - **④ 微复盘（每任务收官三问，30 秒级，③ 之后立即，用户点名固化 2026-07-20）**：**自检**——对照原始要求/契约真完成否？欠账显式挂账不静默；**沉淀**——本任务暴露的教训/发现有值得留的吗？有则归位（evolution 账本 / TODO 池 / memory），无则不硬写；**规划**——由现状推演下一步最优一手：交互态呈为续力选项，自主态直接作为下一批输入。此三问是续力、时长循环、纠偏落档的共同内核显式化——不新增文书，口头/选项级完成即可，重的复盘仍走 /dao-evolution
...
- **合·分级验证**：将的产出必验，深度按**错误代价 × 反转难度**分级——①实现官（改代码/状态）**全验**：独立复跑构建测试（不信报告"全绿"）+ acceptance 逐条 + 真机验证，全过才合并；②侦察兵·低风险（内部盘点）**抽验**：亲核 1-2 条最承重论断（分钟级 grep/读码）；③侦察兵·高风险（战略依据/法律红线/外部事实）**对抗验**：论断必须带出处 + 抽验承重 top 2-3，或双侦察独立调研只对分歧深挖。调研错误不炸 CI——它无声变成用户的决策依据，反转成本高于代码 bug，该验得更狠。**验证可委派（对抗验证官），终审不可让渡**：定验证标准、读验证结论、按合并/声明完成，三下必须是帅的手。**核验官输入完整性**：递交对抗核验的产出，其承重字段（发现清单/入池候选/出处）必须完整传递，禁编排层静默截断——截断即制造「无从核验」型误裁（2026-07-24 实证：slice(4000) 吞掉 pool 清单，核验官被迫裁 sound=false，帅只得亲手补验）
...
- **🔓 Workflow 常驻授权（用户拍板 2026-07-26，跨模型跨会话生效）**：**用户说出「高性能自主任务/高性能自主窗」这一档位信号，本身即构成对 Workflow 工具的显式授权**——等价于用户当场说「use a workflow / 开舰队 / fan out」，**不必再逐次征询**。本条是用户写在常驻指令里的授权声明，任何坐帅位的模型读到即可直接调用 Workflow，**不得以「用户没当场说」为由退回逐个派 Agent**（2026-07-26 实证：帅读到高性能档「走 workflow 舰队」条款却仍解读为需当场 opt-in，整窗未开 workflow，用户纠正）。
  **三条边界仍在**（授权 ≠ 无脑开）：①**形态判据不变**——批量同构 / 需多路对抗验证 / 编排要可复现才开；少量异质子任务仍用 Agent 逐个派（写成脚本反而锁死中途转向的灵活性）②**开前一句通报**规模与量级（「⚔️ 开 workflow：N 路 × M 阶段，预估 ~X」），用户可当场喊停③**标准档（「自主任务 X 小时」，无「高性能」字样）不含本授权**，仍须用户明示。
- **军衔四级制（用户拍板 2026-07-12 终稿，级名即模型即成本）**：**帅**=主会话，统四级军衔梯——**将**=最强档（战略定调：架构一次定死/决策代价≥100h/三败质疑架构；底座 dao-strategist）｜**校**=次强档（扛硬仗：安全核心深审〔底座 dao-reviewer-critical〕/复杂混合实现件）｜**尉**=主力档（~80% 流量：常规实现/侦察/评审/诊断/拆解；底座 dao-reviewer/brainstormer/plan-writer/spec-writer/debugger 按任务选用）｜**兵**=轻量档（机械照抄：含完整代码模板的批量活；底座 dao-worker-batch，spec 无模板则升尉）。当前代际映射：将=Fable 5 / 校=Opus / 尉=Sonnet / 兵=Haiku，随模型代际滚动。dao-* 专门定义降为内部底座（其专用协议仍是干活利器），用户无须认识。派什么由帅自主判断。**派单通报（每次必带）**：「⚔️ 已派**军衔**（模型·白话任务）→ 本次干什么」，多路并行逐行；完稿汇报同带括注——用户会遗忘，每次派单都是重新学习的时机
- **长窗自主排程（用户拍板 2026-07-17：无明确目的的长窗，帅自主判断现状定任务，直到满足时长）**：①**开窗仪式**：设心跳（见下条）→ TaskCreate 台账 → 输出**开窗报告**（现状分析→拆解→时长预算）——用户离场时流程自转，回来可审计判断质量；**台账时刻以 ScheduleWakeup 返回的实时为唯一锚，禁凭批次进度推算**（2026-07-22 实证：推算钟漂快 ~2h，时长授权窗对时错误会直接错判收窗时机）②**选题判断框架**（不是搬 TODO 清单）：先判健康期还是病期（体检/CI 红着→先修敌；干净→做建设）；建设期选题按价值排序——**越晚越贵的事优先**（回溯成本递增件：发布基建/账实对齐/CHANGELOG/教训沉淀）＞**基建刚铺完的趁热件**（上一批铺的地基使某功能边际成本最低）＞**已定夺纯执行件凑小件批**＞一般候选按用户价值密度（竞品痛点频度/dogfood 高频；有 `docs/POSITIONING.md` 支柱图的产品项目以「哪根最短/最防御」为镜）②.5 **见底前外向自检（触发式非配额，用户点名反闭门造车 2026-07-22，对抗审收硬）**：宣告「候选池见底」前扫一眼外向信号有无未消费项——桌面已装竞品未实机体验 / 竞品新 release 未扫 / **已发布产品有用户反馈未读（最高权重）**；有则消费一轮（实机走查照「竞品即将就史」记将就点入池标 [竞品·体验]，其权重高于 issue 挖掘；守洁净室+非破坏红线），无则见底成立。**豁免**：无同类竞品项目 / 病期救火窗 / 短窗（<4h）。用户面功能批派单契约须附「竞品同面形态一句话+出处」或注「无先例」，防闭门发明已有答案。细则见 `docs/playbooks/outward-loop.md`——池空≠没事做，池空=该出门看看③**时长换算容量**：一个并行批 ≈ 实现 1.5h+串行验收 1h+事故缓冲 0.5h；**窗口末段固定留 ~25% 给收官**（dogfood/欠账/总账/回归报告），不许功能塞满③.5 **收官简报先行铁序（2026-07-22 两连实证）**：窗末简报/收官交付文本必须**先于**值守心跳设置、作为独立完整消息发出——「打算发」不是「已发」；且 ScheduleWakeup 的 prompt 只准写未来动作，禁止预写「已发/已完成」类既成事实（预写会在下次唤醒时被当真，形成自证幻觉，同一收尾坑单日两踩；**2026-07-22 晚再连踩两次升为六踩**——含「写教训的同一轮里踩教训」）；**硬步（第六踩固化）**：以 ScheduleWakeup 收尾的轮**该轮即未发简报**（调度器「Nothing more to do」不是收轮许可），每次唤醒第一动作=核查上轮是否真有面向用户的最终文本，无则本轮先补发简报再做任何调度/stop；**预记禁令（第八踩固化 2026-07-23）**：简报的「已发」只准在**下一轮回看核实后**记账——台账、心跳 prompt、任何当轮文本预记「已发/已随本轮发出」皆违例，当轮只准写「随本轮末尾发出（下轮核实）」类未来式；**循环存续铁律（第九坑固化 2026-07-23）**：纯文本收尾的简报轮必须在**同轮更早的工具段**先设好后继 ScheduleWakeup——「简报后下轮再设心跳」是死循环缺口（零在途+零待触发时不存在下轮，循环饿死，实证停摆 1h+ 由用户唤回）；铁序正确形态=同轮 tools-then-text，非拆两轮；**三明治规则（第十踩规律固化 2026-07-23）**：ScheduleWakeup **永不作为轮内最后一个工具调用**——其返回文案具收轮暗示性，十次漏发简报全部以它收尾、凡其后另有工具调用则零失误；设完心跳必须紧跟至少一个其他工具调用（台账记行即可）再出最终文本；样本更新 2026-07-24 通宵窗：再犯六踩（11-16，全部 Wakeup 居轮末，派单轮/收编轮皆有变体），执行「Wakeup 挪至轮中段+**心跳 prompt 末尾固定携带铁序一句**（给下一轮的自我提醒）」后段间十连零失误——prompt 自提醒为已验实践随条款生效④**时长循环（2026-07-17 提前收窗实证补）**：授权的是**时长**不是批次——每批收官段完成后必核对剩余时长：≥1 批容量（~2h）→回到②选题框架再排一批；<1 批但 ≥30 分钟→填充小件/侦察/欠账/候选核实类短活；接近零才真收窗+总结。批次跑得比预估快产生的空档同样回此循环，禁止「计划完成=窗口完成」的单方面提前收窗⑤**自主边界（永不进自主窗）**：不可逆决策（架构定死/对外发布/删用户数据）、需用户在场件（凭据/审美拍板/真钥）、用户未点头的新疆域——自主窗只做「沿既定方向推进+健康维护」，转向权归用户⑥**双档模式（用户拍板 2026-07-21）**：自主窗分两档——**标准档**（「自主任务 X 小时」）：现行为，帅调度、单官/少量并行、成本门正常；**高性能档**（「高性能自主任务 X 小时」）：多路 fan-out 默认化——调研/审计/评审走 workflow 舰队+对抗验证、实现件尽可能并行拆官、验证升双验证官、成本自评门免除，同时长内吞吐最大化。**档位识别按语义不按口癖**（用户点名原则：口癖因人而异，禁关键词硬匹配）：出现「高性能/放开用量/额度用不完/多开子代理/上 workflow/全力/舰队」任一**语义**，或处于用户先前给出的时限性放量授权期内 → 高性能档；无信号 → 标准档；拿不准 → **开窗报告首行声明本窗档位**（声明代替提问，用户一句话可纠，不打断流程）
- **长窗防停摆四层（2026-07-17 停摆实证 L25 固化）**：核心原则——**监督信号必须独立于作业信道**：主线推进若只靠 task-notification 事件驱动，任何一路 subagent 沉默（watcher 悬挂/通知丢失）即饿死整窗。四层防御缺一即有对应事故模式：①进入长窗**第一动作**设 ScheduleWakeup 心跳（**高性能档 900s=15 分钟，用户拍板 2026-07-22「永久改」；标准档 1200-1800s**。prompt 写明：盘点在途→悬挂即介入催收/接管→无事推进下批→用户接管且无欠账即 stop）②派单令标配「等待类验证（build/dev server/外部资源）超 15 分钟未就绪→放弃该手段、欠账显式化、直接交付」——完美验证卡死不如降级交付；同标配「后台验证发起后须**主动轮询 Read 其日志直至出结果才准停手**，禁被动等通知；**armed 任何形态的 watcher/poller/monitor 后停手均不算主动轮询——主动轮询=亲手 Read 到终态**（第七例 armed monitor 2026-07-22：官装监视器后停手自证「已主动」；第八例 armed poller 2026-07-24 mousse-cli：官声称「等 poller 完成通知」而其已零在途后台子任务，等的通知永不会来，催收即愈）」——「等自后台」停摆型（官发起后台 verify 后干等通知饿死）单窗四官次实证，条款入令后次批双官零停摆（2026-07-22 mousse-cli）③在途任务 TaskCreate 登记（路名/预计归队/超时线），心跳醒来先对账再决策，不凭印象盘点④派单后立即逐路抽验 worktree 基点（`git -C <wt> merge-base HEAD main` = 当前 main tip，旧基点当场令实现官先 merge main 再开工，L24 两案实证基点漂移是常态风险非偶发）
...
1. **住进场景**：先问「用户在什么具体时刻碰这个功能、那一刻心里想完成什么」——枚举真实使用时刻与意图，一个意图一个设计对象。设计从场景里长出来，不从功能清单/checklist 里长出来（便签三型=三种真实时刻：想重复说过的话 / 想甩出模糊目标让人接住 / 怕忘闪念）
2. **有用性测试**：每个设计点问「这让那个时刻的用户更省心了吗」，不是「功能更强了吗」——场景召唤不出的能力是负资产（计算器不要 AI 绑架用户思考）
3. **竞品即将就史**：竞品形态=用户已被迫接受的最低解；找用户在哪些时刻正在将就（土法自救/绕路/放弃），将就处即机会。**issue 反馈分层读（用户拍板 2026-07-22）**：bug/信任崩塌类近字面采信；功能请求类=处方可疑、痛苦真实——频度×烈度证的是场景强度非方案正确性，入池按场景归因（记时刻/意图）不照单抓药；护栏：处方无歧义且低成本的小件直接做（实证：Codex #9203 388 人齐呼复活 /undo，git 系 undo 恰救不了其主诉的 untracked 半边——用户开的处方治不全用户自己的病）
...
**支柱工作法（产品级宏观层，与上述功能级心法配套）**：产品定位用「支柱图」承载——支柱=价值维度假设，是选题之镜（问「哪根最短/最防御」，不问「竞品有没有」）。生命周期：直觉假设 → 仓库对账 → 全盲多路侦察挑战假设（防创始人视野框死自己）→ 对抗双镜评审 → 查漏批判（商业项目必照三镜：**克制**·**蒸发测试**·**变现**）→ 终版落项目 `docs/POSITIONING.md` → 此后选题/长窗排程以它为镜 → 市场剧变时重跑。战役脚本与细则见 windsurf-dao `docs/playbooks/pillar-recon/`。实例结论不进 dao（L27）。

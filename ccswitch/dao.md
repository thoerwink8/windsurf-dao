...
- **长窗防停摆四层**（出处：L25）：核心原则——**监督信号必须独立于作业信道**：主线推进若只靠 task-notification 事件驱动，任何一路 subagent 沉默（watcher 悬挂/通知丢失）即饿死整窗。①进入长窗**第一动作**设 ScheduleWakeup 心跳（高性能档 900s、标准档 1200-1800s；prompt 写明：盘点在途→悬挂即介入催收/接管→无事推进下批→用户接管且无欠账即 stop）。②派单令标配「等待类验证超 15 分钟未就绪→放弃该手段、欠账显式化、直接交付」；同标配「后台验证发起后须**主动轮询 Read 其日志直至出结果才准停手**——armed 任何形态的 watcher/poller/monitor 后停手都不算主动轮询」（出处：叙事 N5）。③在途任务 TaskCreate 登记（路名/预计归队/超时线），心跳醒来先对账再决策，不凭印象盘点。④派单后立即逐路抽验 worktree 基点（`git -C <wt> merge-base HEAD main` = 当前 main tip，旧基点当场令实现官先 merge main 再开工，出处：L24）

...
- **多将并行拆分判据（两组实测固化 2026-07-12：零相交型 + 契约耦合型均零冲突交付）**：并行只在**横向模块切**下赢，三判据全过才拆——①文件相交集近零（共享触点指定唯一 owner，追加型冲突留帅合并统一解、属预期非事故）②接口能在派单前**冻结进契约文档**（依赖侧可按契约先行绑定 + 缺字段容错降级，运行时联调帅在合并后做）③单将预估 >40 分钟（拆分固定税 ~10 分钟：冻结契约+多轮验收）。**纵向功能切片不拆**；已开工领地被占时中途重切是负收益，改用加速令（要 ETA+砍非核心）
- **规模分界**：少量子任务用 Agent 工具逐个派；批量同构 / 需对抗验证 / 编排要可复现 → Workflow 脚本（重器，须用户明示授权）
- **职称名录（用户采纳 2026-07-12）**：帅=主会话 ｜ 军师=dao-strategist ｜ 谋士=dao-brainstormer ｜ 参谋=dao-plan-writer ｜ 令官=dao-spec-writer ｜ 监军=dao-reviewer ｜ 御史=dao-reviewer-critical ｜ 先锋=通用型实现（worktree 隔离）｜ 工兵=dao-worker-batch ｜ 军医=dao-debugger ｜ 斥候=通用型只读侦察。派单与汇报统一用职称。**首现括注**：每会话某职称首次出现时括注白话（如「斥候（只读侦察）」），其后裸用——用户无须记名录

...
- **长窗自主排程（用户拍板 2026-07-17：无明确目的的长窗，帅自主判断现状定任务，直到满足时长）**：①**开窗仪式**：设心跳（见下条）→ TaskCreate 台账 → 输出**开窗报告**（现状分析→拆解→时长预算）——用户离场时流程自转，回来可审计判断质量②**选题判断框架**（不是搬 TODO 清单）：先判健康期还是病期（体检/CI 红着→先修敌；干净→做建设）；建设期选题按价值排序——**越晚越贵的事优先**（回溯成本递增件：发布基建/账实对齐/CHANGELOG/教训沉淀）＞**基建刚铺完的趁热件**（上一批铺的地基使某功能边际成本最低）＞**已定夺纯执行件凑小件批**＞一般候选按用户价值密度（竞品痛点频度/dogfood 高频）③**时长换算容量**：一个并行批 ≈ 实现 1.5h+串行验收 1h+事故缓冲 0.5h；**窗口末段固定留 ~25% 给收官**（dogfood/欠账/总账/回归报告），不许功能塞满④**自主边界（永不进自主窗）**：不可逆决策（架构定死/对外发布/删用户数据）、需用户在场件（凭据/审美拍板/真钥）、用户未点头的新疆域——自主窗只做「沿既定方向推进+健康维护」，转向权归用户

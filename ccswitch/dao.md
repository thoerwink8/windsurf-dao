...
- **职称四级制（用户拍板 2026-07-12，级名即模型即成本）**：帅=主会话，统四级——**军师**=最强档（战略定调：架构一次定死/决策代价≥100h/三败质疑架构；底座 dao-strategist）｜**将**=次强档（独当一面硬活：安全核心深审〔底座 dao-reviewer-critical〕/复杂混合实现件）｜**兵**=主力档（~80% 流量：常规实现/侦察/评审/诊断/拆解；底座 dao-reviewer/brainstormer/plan-writer/spec-writer/debugger 按任务选用）｜**工兵**=轻量档（机械照抄：含完整代码模板的批量活；底座 dao-worker-batch，spec 无模板则升兵）。当前代际映射：军师=Fable 5 / 将=Opus / 兵=Sonnet / 工兵=Haiku，随模型代际滚动。dao-* 专门定义降为内部底座（其专用协议仍是干活利器），用户无须认识。派什么由帅自主判断。**派单通报（每次必带）**：「⚔️ 已派**级名**（模型·白话任务）→ 本次干什么」，多将并行逐将一行；完稿汇报同带括注——用户会遗忘，每次派单都是重新学习的时机

...
- **长窗防停摆四层（2026-07-17 停摆实证 L25 固化）**：核心原则——**监督信号必须独立于作业信道**：主线推进若只靠 task-notification 事件驱动，任何一路 subagent 沉默（watcher 悬挂/通知丢失）即饿死整窗。四层防御缺一即有对应事故模式：①进入长窗**第一动作**设 ScheduleWakeup 心跳（**高性能档 900s=15 分钟，用户拍板 2026-07-22「永久改」；标准档 1200-1800s**。prompt 写明：盘点在途→悬挂即介入催收/接管→无事推进下批→用户接管且无欠账即 stop）②派单令标配「等待类验证（build/dev server/外部资源）超 15 分钟未就绪→放弃该手段、欠账显式化、直接交付」——完美验证卡死不如降级交付；同标配「后台验证发起后须**主动轮询 Read 其日志**收尾，禁被动等通知」——「等自后台」停摆型（官发起后台 verify 后干等通知饿死）单窗四官次实证，条款入令后次批双官零停摆（2026-07-22 mousse-cli）③在途任务 TaskCreate 登记（路名/预计归队/超时线），心跳醒来先对账再决策，不凭印象盘点④派单后立即逐路抽验 worktree 基点（`git -C <wt> merge-base HEAD main` = 当前 main tip，旧基点当场令实现官先 merge main 再开工，L24 两案实证基点漂移是常态风险非偶发）

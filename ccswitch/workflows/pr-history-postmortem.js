export const meta = {
  ...
  whenToUse:
    '需要 args: {repoPath}(必填,不设默认——跨项目复用的资产不该内置某一个仓的路径)。' +
    '可选 args: {ghRepo:"owner/name", prRange:{from,to}, lenses:[...], knownDefects:[...], ' +
    'clauseFile:"docs/rules/xxx.md", ledgerFiles:[...], goal:"...", model, verifyModel}。' +
    '何时跑:①项目积累 100+ PR 且全部由 agent 产出,想知道「我们怎么干活出的问题」(不是产品 bug);' +
    '②立了一批派单条款/流程规则,想验证它们入库后到底防住了什么;③长窗排程时想从历史里找选题;' +
    '④**好实践收割的「量化聚合」半边**——另一半(单次叙事)由 `dao-harvest` 覆盖,见下。' +
    '⚠ **它只是好实践的一半**:2026-07-27 首轮实测,本路的量化候选与 `dao-harvest` 的叙事候选' +
    '**重合度为 0**(两个方向都零重合,互不相交不是子集)——「N 个 PR 都有某个毛病」只在把全史排成' +
    '一列后才存在,单次叙事源结构上看不见它;反之「某官当场拒绝编造一个不可达的映射」不留统计痕迹,' +
    '本路结构上看不见它。连带规律:**量化聚合天然产出形态类候选,单次叙事天然产出判断类**' +
    '(能被计数就意味着有可机械识别的特征,而那特征本身就是现成的机检判据)。' +
    '⇒ 收割场景下两个都要跑(同一 repoPath),只跑一个会系统性漏掉一整类;' +
    '两份候选并到同一轮后**互相之间也要判重**,同一个病可能一边以「某次某官踩了」出现、' +
    '另一边以「N 个 PR 都有」出现,那种情况合成一条且 n 取聚合那边的数字(真实复发次数下界)。' +
    '改造自 2026-07-27 mousse-cli 单次专用版(硬编码仓路径/PR 上限/该仓当日已知缺陷清单),' +
    '方法论骨架(五镜 + 对抗核验 + 「入库后归零必须排除没机会发生」)原样保留。',
  phases: [

...
🔴 **但这一节搬完，那个目录还是不安全 —— 数字摆在这里，不放折叠段里**
（2026-08-06 实测，只数条数不看值）：

| 这一节搬走的 | 搬完之后**还留在那个目录里**的 |
|---|---|
| **1 个**账号（2 个键：`GITHUB_USER` / `GITHUB_PASSWORD`） | `data\trial-accounts.json`：**112 条**记录，`password` 字段**非空 112/112**、`email` 非空 112/112 |
| | `data\cloud-accounts-export.txt`：**356 行**，每行恰好 2 个字段且都含 `@` |
| | `data\cloud-trial-accounts.txt`：**140 行**，同上形状 |
| | `.auth\` 下 **3 套** Chromium 登录态目录（`browser-profile` / `chrome-profile-9222` / `chrome-profile-9333`），三套都命中 `Cookies` `Login Data` `Web Data` `Local State` 等八项 |

**⇒ 搬走的是 1 条，留下的是 112 条起。**「复制一次那个目录 = 复制一次明文口令」这句话，
搬完之后**依然成立，而且成立得更厉害**。已挂账 **#137**，不在这一节的射程里。

⚠ 那两个 `.txt` 的第二个字段**是不是口令，这是按「每行恰好 2 个 token + 含 `@`」的形状推的，
不是确证**（确证要看值，刻意不看）。所以「608 条」这个上界可能偏高；
但 `trial-accounts.json` 那 **112 条的 `password` 非空**是实打实数出来的，这一半是硬的。
...
**私钥落在 `%AppData%\sops\age\keys.txt`**（凭据根**之外**，理由见下面折叠块）。
那正是 sops 在 Windows 上自己找钥匙的地方，所以**不用设任何环境变量**。

想让私钥和密文住一起（一个文件夹拷走即换机）就加 `-KeyLocation Portable` ——
那条路要设一个 `SOPS_AGE_KEY_FILE`，脚本会把命令打给你，加 `-SetUserEnvVar` 可自动设。
...
**你在乎的那一件已经成了**：密钥不在任何项目工作树里 —— 凭据根在 `%USERPROFILE%` 下，
`Portable` / `Separate` 两种模式都满足它。**下面这个开关不参与那一件的成败**，它只是在上面
再加一层，所以按「加的这层值不值」看就行。

**默认 `Separate`（你 2026-08-06 拍的）**：私钥放 `%AppData%\sops\age\keys.txt`，
凭据根里**只有密文、没有钥匙** ⇒ 有人专门拷 `.dao-secrets` 那个文件夹时，他拿到的是
一堆打不开的东西。之所以能这么选，是因为你说了**私钥丢了不要紧**（重新生成 + 把那几个值
重填一遍即可，那些值本来都能重新申请）—— 而 `Portable`「一个文件夹拷走即换机」这个卖点，
恰恰建立在「私钥很宝贵、要小心带走」这个前提上，前提不成立，卖点就归零了。

🔴 **它买到的那一格有多大，照直说**：实测 `%AppData%` **就在** `%USERPROFILE%` 里面
（`C:\Users\<你>\AppData\Roaming` vs `C:\Users\<你>`）⇒ **整个用户目录被备份 / 同步 /
做镜像的时候，私钥和密文照样一起走**。`Separate` 只挡「专门拷 `.dao-secrets` 这一个文件夹」
这一种情况。真要挡住前者，得把私钥挪出 `%USERPROFILE%`，**脚本没做这件事**。

**关于丢钥匙**：`%AppData%\sops\age\keys.txt` 没有第二份、没有找回通道，丢了所有加密文件
就永久打不开 —— 但按你的口径这**不是灾难**，是「重来一遍」：重新跑 init 生成新钥匙、
把那几个值重填。嫌麻烦就单独复制一份到密码管理器或离线介质。

**还有两件本节没解决、已单独挂账的**：

- **#136** —— resume-project 的 JWT 密钥和管理员口令有**静默的弱默认值**，
  而叫人配置的那个 `.env` **从来没被程序读过**。搬不搬凭据，那个默认值都在。
- **#137** —— devin-credit-claimer 目录里**还有成批账号口令和浏览器登录态**没搬。
  它们不是 `key=value`，dotenv 的搬法套不上去。**具体是多少条，见本节开头那张表**
  （2026-08-06 起数字只写在正文，这里不复述 —— 抄一份就多一个会漂移的点）。
  ⇒ **别把「4 处清完」读成「那个目录安全了」。**
...
**默认值改成 `Separate` 那一批另有一套常驻回归网**（2026-08-06，`tests/dao-secrets.tests.ps1`
场景 13/14/15，跑的是**桩**不是真 sops）：Separate / Portable / 默认值三条各一格，
断言打在私钥落在哪个盘上位置与 ACL 的继承状态上；9 个 mutation 变体给出 5 个互不相同的红集。
**它不需要动你真实的 `%APPDATA%`** —— 覆写的是测试进程自己的环境变量，且有一条负控专门核
「真 `%APPDATA%\sops\age\keys.txt` 的存在状态没被跑动改变」。
...
- 安全模型本身（`Portable` 把私钥和密文放一起、`Separate` 只挡「专门拷这一个文件夹」、
  丢钥匙要重来一遍、`_backup` 是明文）**不是靠实跑能验的**，它是取舍，见上面那个折叠块。
- **默认值改成 `Separate` 之后，没有人用真 sops 从头到尾再跑一遍** —— 上面那条「两种模式各一次」
  是改默认值**之前**跑的（那次两种模式都是显式传参，所以两条路径本身跑过；没跑过的是
  「不传参数时走哪条」这个新默认）。常驻回归网覆盖了它，但那是桩不是真 sops。

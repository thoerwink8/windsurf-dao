...
## 二、把开会话体检的超时从 10 秒抬到 32 秒（2026-08-04 你拍板，约 1 分钟）

> 🔴 **这个数 2026-08-10 从 30 改成了 32，你照做的两步也一起改了**。
> 你 08-04 拍的是「抬到 30 秒」，当时的目标只是**别撞上超时**，30 够。
> 但 08-10 起报文里多了一句带 `⚠` 的告警（见下面「不做会怎样」），
> **抬到 30 秒它不会消失** —— 30 秒真正能用的是 28500 毫秒，而代码里那两个 30000 仍然够不着。
> **分界线在 31.5 秒，取整就是 32。** 做法一个字没变，还是跑一行命令 + 切一次 provider。
> （出处：PR #277 对抗复核 —— 原来的步骤会让你「照做完、核对通过，而那个 `⚠` 一个都不少」。）
...
```
powershell -File D:\frank\windsurf-dao\_tmp\raise-scaffold-timeout.ps1 -NewTimeout 32
```

🔴 **`-NewTimeout 32` 要带上**：那个脚本自带的默认值是早先定的 30（本机那份已一并改成 32，
但它住在 `_tmp/` 里、**不进 git**，换台机器就没有了）⇒ 显式写出来才不靠默认值。
...
`timeout` 是不是 `32`。⚠️ **没切之前它仍是 10 —— 那不是脚本没生效，是还没下发。**
（看到 `30` 说明第 1 步漏了 `-NewTimeout 32`，重跑那一行再切一次即可。）
...
**2026-08-10 起多了一格你看得见的**（issue #273 G4，你当天拍板「算发现，按同规修」）：
体检报文里那句「内层超时常量 … 都大于有效截止线」**从此带 `⚠`**。它此前不带，于是每次会话
都被折进「N 行全绿」那一句里——一条每次都出现、却从来没人看见的告警。带上 `⚠` 之后它会
**阻断折叠**：只要那几个常量还够不着，开场那段就不再显示「N 行全绿」，而是逐条展开。

🔴 **照直写一格容易踩空的，也正是上面把 30 改成 32 的理由**：**抬到 30 秒不会让这句话消失**。
30 秒的有效截止线是 28500 ms，而 `DEAD_GATES` / `PROVIDER_HOOKS` 那两个常量是 30000 —— 仍然
够不着（那正是 PR #130 修好的那一格：旧判据下它们会在 30 秒时"消失"而实际仍够不着）。
**分界线是 31.5 秒，取整 32** —— 上面第 1 步那条命令带的 `-NewTimeout 32` 就是这个数。

另一条路是**把代码里那几个超时常量降下来**，但**注意是五个不是两个**：注册不动（10 秒）时
有效截止线只有 8500 ms，`CLAUSE_CHECK` / `BUDGET` / `MEMORY_REFS` 那三个 20000 同样够不着 ⇒
**只降 `DEAD_GATES` / `PROVIDER_HOOKS` 那两个 30000，仍剩三个够不着、告警照旧在**。
（这句话此前写成「或者把那两个 30000 降下来」，读起来像一条独立可走的路 —— 它只在
「你已经把注册抬到 30 秒」这个没写出来的前提下才成立。2026-08-10 PR #277 对抗复核实算订正。）

**走哪条归你/帅定，本批没替你定** —— 这件事已挂在 **issue #281**（有 owner、有解冻条件，
在那张单上回一句就算拍了）。
...
**跑完看一眼退出码**（`echo $LASTEXITCODE`，你 2026-08-07 拍的分档，issue #148）：

- `0` 全成 —— 钥匙、规则文件、自证、权限收紧，四件都成了
- `2` **钥匙建好了，但权限没收紧** —— 只会在**不支持 ACL 的盘**上出现（U 盘 exFAT、
  网络盘之类）。脚本**不拦你**，最后一屏会红字点名是哪几个目录；要不要换个 NTFS 盘
  重跑（`-SecretsDir <新路径>`）由你判断
- `1` 真失败 —— 该做的事没做成，别往下走第 3 步

改这一档之前，`2` 那种情况也是 `0`：屏幕上有红字，但**退出码上「权限没收紧」和
「一切正常」一模一样**。红字给你看，退出码给脚本看，两边都得说实话。

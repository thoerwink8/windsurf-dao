...
🔴 **照直写一格容易踩空的**：**抬到 30 秒不会让这句话消失**。30 秒的有效截止线是 28500 ms，
而 `DEAD_GATES` / `PROVIDER_HOOKS` 那两个常量是 30000 —— 仍然够不着（那正是 PR #130 修好的
那一格：旧判据下它们会在 30 秒时"消失"而实际仍够不着）。要让它彻底消失，注册得 **≥ 32 秒**，
或者把那两个 30000 降到有效截止线以内。**这一格归你/帅定，本批没替你定。**

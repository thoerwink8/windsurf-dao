...
不动清单点名的 19-41 行——那 23 行原样留在源文件、逐字节未动，只是前面少了一行、整体上移一格，
改动后位于 18-40 行。**本节所有行号都是"迁出时刻"的行号，源文件一改就漂**——权威坐标是本档随
PR #251 落地的那个 commit，锚点形态归 issue #262 件㈣ 待拍板）；② `resolveRegisteredTimeoutMs`

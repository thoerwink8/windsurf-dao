...
**会话生命周期以本仓本文为准**（到期不续 / 溢流 / 是否投新会话，见下节第 3 条）；
外仓固定提交只引用网关侧并发两口径与 opencode `header_override`，不是同一份最终生命周期口径。
实现三件套（上限 / 顺位分流 / 熔断）与病腿贴标的代码验收在实现单，不在本文档 PR。
网关侧同日请求级文档与 header 修复：`thoerwink8/ai-gateway-stack@a55e9c45822100ec23bbc92c2d185ee69a03c650`
（[ROUTING 并发两口径 + header_override](https://github.com/thoerwink8/ai-gateway-stack/blob/a55e9c45822100ec23bbc92c2d185ee69a03c650/docs/ROUTING.md)）。
该提交写于 05:47，早于 06:11 拍板；其 ROUTING「渠道生命周期」段仍写 opencode/commandcode
「到期不续、不投新会话」，是中间口径，不要照那一段调度。
...
2. **上限是「会话级」数字**，填路由表「腿」节 `并发上限` 字段（外部账号事实，人填；
   与「机器余量不许填死数」不冲突——那条管的是机器容量）。
   填数依据必须是真实派工的会话死因 / 账号可用性，**不得把 HTTP 短探针或长流探针直接当会话容量**。
   09-08 表里的填入初值是当日拍板快照；其中 pqapi「CLI 3-4」已于 09-09 标成未证实观察（见「后续勘误」）。
3. **渠道生命周期**：
   ...
   - 外链 `ai-gateway-stack@a55e9c4` 生命周期段与本条不一致时，**以本条为准**。
...
09-09 实现单把审官全灭的真因归到这一道（及 mirasim relay 过载），不是渠道 429，见「后续勘误」。

## 并发两口径（混谈会自相矛盾）

- **请求级**：一瞬间能同时处理几个一次性 HTTP 调用（2-3 秒/个）。
- **会话级**：同时活着几条 agent 长会话（几十分钟持续流+工具回合）。供应商按账号另行计量
  （concurrent runs）。长流探针（N 条并行流式长输出）只是 HTTP 层可观测代理，**不等于**
  真实 agent 会话容量；`max_tokens=8/16` 的短探针更不能外推长会话。
  09-08 曾把「codex CLI 3-4」写成更紧的墙，09-09 已撤回该观察（见「后续勘误」）。
  **最终判据永远是真实派工的会话死因统计（看板回调），探针只定初值。**

## 实测数据与填入路由表的初值

下表是 **2026-09-08 当日快照**，数字不回溯改写。会话经验值里的「CLI 3-4」按未证实观察保留。
机器可读落点：`docs/decisions/2026-09-08-channel-caps.json`（dao-check 对账读它，不解析本表）。

| 渠道（组） | 请求级实测 | 长流实测 | 会话经验值（09-08 观察） | **填入初值（会话级）** |
|---|---|---|---|---|
| pqapi（codex sol/luna） | ≥8 | 6 | CLI 会话 3-4（**未证实观察**，09-09 撤回）；当天三次中继死 | **2**（09-08 初值；原依据「CLI 3-4 / 中继更紧」已撤回，不是渠道容量） |
| windsurf（luna/kimi 等 48 模型） | ≥10 | 8 | 中间口径曾记用户经验 4，已作废 | **6**（06:11 拍板；长流 8 全绿是 HTTP 代理，不是真实会话证明；风险由看板死因统计兜底） |
...
方法（可复述，不依赖本仓脚本）：请求级是对网关渠道并行打短 HTTP（当时 `max_tokens=8`）；
所谓会话级代理是 N 条并行流式长输出。两者都是 HTTP，都不是真实 agent 长会话。
一次性探针脚本当时写在本机临时目录，**不在本仓库、本提交也没有**，不能从本 HEAD 复跑实验。
限制：HTTP 短探针 ≠ HTTP 长流 ≠ 真实 agent 会话。09-08 把「codex CLI 3-4」写成更紧的墙，
该观察已撤回。初值以看板死因回调为准。

## 后续勘误（2026-09-09，不改写上面的 09-08 快照）

09-08 当晚把「Codex 容量 3-4、一挤全 429」和「长流探针 ≈ 会话容量」写进本底稿。
次日用户质疑后复核（issue #1145 [comment 5602384835](https://github.com/thoerwink8/windsurf-dao/issues/1145#issuecomment-5602384835)），已撤回该运行时证据：

- 态势快照里的 `429` / `at capacity` 全部出自 issue/PR 正文回声，不是运行时记录；熔断表 /
  健康表 / 账本 / admission / preflight **零条真 429**。
- 「3-4」来自当时 new-api 里 gptpool **只剩一条可用账号**（约 3 个并发），是账号可用性，
  不是渠道容量。`Selected model is at capacity` 是上游无可用账号时的转译文案。
  上表「CLI 会话 3-4」按**未证实观察**保留，不当事实引用。
- 09-08 请求级探针是 `max_tokens=8` 短 HTTP；09-09 梯度复核是 `max_tokens=16` 短 HTTP。
  二者都不能证明几十分钟的 agent 长会话容量。长流探针（并行流式长输出）仍是 HTTP，
  同样不是真实会话。
- 已合并实现 PR #1169 把真因改写为：审官长流打在同一台网关机上的 CPU 过载，以及
  mirasim relay 过载（`relay-ticket: mint error` abort）。总并发长流 ~12-14 即触发
  90% 保护整机 503。立论从「防上游渠道 429」改为「防 relay 过载 + 防同时起一堆会话的雪崩」。

对拍板第 2 条的影响：上限仍然是会话级数字、仍由人填进路由表；**依据不再是「CLI 3-4 /
长流探针」**。09-08 表里的填入初值是当日拍板快照（windsurf=6 仍以 06:11 为准）；
pqapi 填 2 的原依据已撤回，该格只表示当日保守初值。后续实现按网关机天花板、relay 过载
与「待填宁可排队」取值，见 #1169 合并前更正，不以本表 3-4 为因果。
...
- 实现单：渠道并发三件套 + 病腿贴标（已合并 PR #1169）；装载面自愈；draft 泵（后两项另开，不由本文档 PR 关单）
- 跨项目射程：dispatch `--repo`（另单）
- 看板：既有 board-watch 表
- 网关侧（只引并发两口径 + 30-channels.sh `header_override`，**不引生命周期段**）：
  `thoerwink8/ai-gateway-stack@a55e9c45822100ec23bbc92c2d185ee69a03c650`
- 09-09 容量勘误：https://github.com/thoerwink8/windsurf-dao/issues/1145#issuecomment-5602384835

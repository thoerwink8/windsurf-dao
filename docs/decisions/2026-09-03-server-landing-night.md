...
当晚关了 **34** 张。复算（亲手跑过，stdout 为 `34`）：

```bash
gh issue list --state closed --search 'closed:>=2026-09-02 closed:<2026-09-02T19:00:00Z' --limit 100 --json number --jq length
```

必须带 `--limit 100`：`gh issue list` 默认只出 30 条。上限钉在 `19:00Z`，因为当晚清理在 17:54–18:40Z 收束；不钉的话会把之后关的单算进去。含 #797/#799/#801 完工关单。#811 初稿写「37 张」是估数，已纠正；不以仍 OPEN 的单补差。名单不在本页复述，三类依据见上表与各单关单评论。

...
- 大版本一个管理员确认即可。
- 临时模型策略（复看日 2026-09-13，grok 新模型发布后）：**写码全走 grok-4.6**；审官仍换厂商（gpt-5.6-sol，全局约定「审查换厂商」不动）；DS 降为辅助。三环节「方案-代码-审核」：方案与代码同一工人（grok）完成，**worker-done 前必须做一轮自查**（跑目标测试 + dao-check + 对照验收标准逐条打勾），审官只审自查之后的版本。
...
- 本单只落文件与检查；发布列车 / 确认闸的机制接入在 #800，这里不改脚本行为。
- 「改规则」含协作约定、本决策文档、`docs/release-policy.json`、`docs/model-routing.json`。
- 审官选型不动。
- 配置值只认 `docs/release-policy.json`，改那份文件走 PR。
...
工人交卷前自查落在 `host/skills/dispatch/templates/soldier-book.md`「交卷前自查（必做）」。

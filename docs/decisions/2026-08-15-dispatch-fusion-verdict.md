...
- **flow.mjs 瘦身保留**：删「发现完工」职责（让位门铃），轮询间隔 90s→300s 降为备份通道；保留审读状态机、判定行解析、存量重放、待帅常驻行；新增敏感路径越权报警（PR diff 触碰 docs/global-CLAUDE.md、CLAUDE.md、host/skills/、scripts/dao-check.mjs 且 PR 正文未声明 → 报警行，此前越权完全无自动检测）。

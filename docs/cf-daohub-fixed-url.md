...
1. **经 hub 发现插件最新地址**（hub 常驻，IDE 没开它也在）：
   ...
   curl -s -X POST https://encircle-wasting-paging.ngrok-free.dev/api/exec-sync \
     ...
     -d '{"agent_id":"DESKTOP-GET3DBC","cmd":"powershell -NoProfile -Command \"Get-Content $env:USERPROFILE\\.dao\\cf-hub-conn.json -Raw\""}'
   ```
...
> 本机（DESKTOP-GET3DBC）通过 ngrok 把 CF-DaoHub 暴露成一个**永久固定的公网地址**，并在登录时自启。以前用 Cloudflare Quick Tunnel URL 每次重启都变，故用固定 ngrok 域名解决。现作为插件的兜底 + 地址发现通道保留。

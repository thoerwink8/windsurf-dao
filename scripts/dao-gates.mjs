...
//
// ── 治的是什么病 ─────────────────────────────────────────────────────────────
// 交付前每个官各自手跑 5 道闸：gen-clause-index --check / gen-guarded-files --check /
// dao-smoke / check-mutation-anchor / check-clauses-structure（全量），各自复制粘贴
// 5 段输出、各自心算「这算不算过」。机械、零判断、且极易漏跑一道——本文件把它收成一条命令。
//
...
//      这里的 5 道闸是 5 个**异构**入口（不同脚本、不同 flag 约定、不同退出码词汇表），
...
//      不顺带发明一个新协议）。
//   ㈡ **串行执行，不是为了性能**：5 道闸依次跑，总墙钟≈5 道之和（`check-clauses-structure`
//      全量模式本身就要 55-81s，是这里边最慢的一道）。选串行是为了和 `run-tests.mjs` 的
//      既有先例一致（避免多个 powershell/node 子进程互抢 CPU 导致耗时判断失真），且这本来
//      就是「交付前跑一次」的场景，不是热路径。
//
// ── 跑法 ────────────────────────────────────────────────────────────────────
//   node scripts/dao-gates.mjs          跑全部 5 道闸，输出汇总表 + 末行 DAO_GATES_SUMMARY
//   node scripts/dao-gates.mjs --list   只列 5 道闸的名字与 note，不执行（自更新：新增/改闸
...
// ── 真实 5 道闸（唯一真相源；改闸清单改这里，不改 CLAUDE.md）────────────────────

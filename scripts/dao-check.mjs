...
import { judgeHarvest, inspectHarvestFixtures } from './lib/harvest-check.mjs';
import {
...
function checkHarvestLive() {
  ...
  const got = runGhJson(['pr', 'list', '--state', 'merged', '--limit', '30',
  ...
  if (problems.length) {
    fail(
      `回流段没人接 ${problems.length} 处`,
      '合并时收口官/帅要接单：段内回写「回流单：#N」或「已回流：<sha>」或「不回流：<原因>」（#888）',
      problems.slice(0, 6).join(' '),
    );
    return;
  }
  green(`回流段 live：近 7 天 ${v.scanned} 个已合并 PR，回流段全有受理证据（${v.accepted.length} 条）`);
}

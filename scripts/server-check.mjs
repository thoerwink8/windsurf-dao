...
/**
 * (25) mirasim 执行体健康：把 `--health --json` 的出口译成三态。
 ...
 * 编号接 master 已占用的 (24) 用量特权副本（#1231）。
 */
...
const CHECKS = [
  ...
  ['(25) mirasim 执行体健康（版本/relay 模式/额度窗，#880 卡 D）', () => {
...
function selfTest() {
  ...
  // #880 卡 D / PR #885：违规 relay/健康样本必须经 (25) 入口拦下，不能只在底层纯函数里红。
  ...
  if (miraRed.state !== RED) {
    failures.push(`故意 available:false 应经 (25) 判红，实际 ${miraRed.state}：${miraRed.detail}`);
  }
  ...
  if (miraBogus.state !== UNKNOWN) {
    failures.push(`故意路由 bogus 应经 (25) 判 unknown，实际 ${miraBogus.state}：${miraBogus.detail}`);
  }

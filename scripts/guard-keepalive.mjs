#!/usr/bin/env node
// scripts/guard-keepalive.mjs —— 帥位触发保活 watchdog + flow（#683 计划任务版 → #693 改 hook 触发）
//
...
// 用法：
//   node scripts/guard-keepalive.mjs --once           检查并按需拉起（唯一入口；也是无旗标默认）
...
function printUsage() {
  ...
  --once           检查 watchdog/flow，不在则拉起（唯一入口；也是无旗标默认）

...
// 同名即同一事件，跳过安全）。跨机汇聚不走 git，走按需拉取（设计见
// docs/dianjiangtai-design.md A 节）：`node scripts/ledger-sync.mjs --from <ssh 别名>`
// 把别的机器的事件汇聚到本机同一落点，判据在 scripts/lib/ledger-sync.mjs（幂等，同名跳过）。

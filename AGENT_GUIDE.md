...
**Orca 共存裁决**（2026-08-10 · issue #299；**2026-08-11 修订**）：本仓是 Orca 托管仓，装进来的 `orchestration` / `orca-cli` 两个 skill 与 dao 在派活/交接/建树等 6 处重叠、2 处硬冲突。**2026-08-11 用户拍板编排底座迁向 Orca**——裁决从「并存听谁」修订为「迁移与过渡期规则」，见 `docs/ops/orca-dao-conflict-ruling.md` 顶部修订节——在本仓派单/交接/建 worktree 前必读那一纸（过渡期一句话：迁移落地前仍 dao 为默认，Orca 走显式）。
...
| 层级 | Agent | 职责 |
|---|---|---|
| 主会话 | 自己 | 分解 / 委派 / 合成 / 验证 / 兜底，不亲执批量实现 |
| 战略 | strategist | 架构定调、卡死攻坚（决策代价 ≥100h 才召） |
| 深审 | reviewer-critical | 核心模块的对抗性评审、复杂混合实现件 |
| 常规 | brainstormer / plan-writer / spec-writer / reviewer / debugger + 实现与侦察 | 主力约八成流量 |
| 机械 | worker-batch | 严格按含完整模板的 spec 执行，零自主判断（无模板就升一档） |

> 🔴 **固定的「哪一层配哪个模型」表已于 2026-08-11 用户拍板退役**（原话大意：档位=固定模型表的时代结束）。
> 现在谁干什么由三样东西**现算**：任务性质 × 开机探测到的可用执行体 × 成本，
> 拟好方案呈用户拍板再开派——判据与提问形态在 `ccswitch/rules/dao-dispatch.md` §二。
> 那一大段「哪天把哪一档改成了哪个模型」的取值订正史归 git 历史，本文件不再维护它。

主会话不写死模型——天然继承当前会话的模型，换代零改动。
用较轻模型坐主会话时即官方 Advisor 变体：遇高难决策临时召一个更强的来咨询即可。

**官种型 agent（2026-08-07 · issue #122 ②）**：`dao-implementer` / `dao-adversary` / `dao-scout` / `dao-dogfood`。
它们**不是又一批能力档**，是给 `agent_type` 带上「这一路是来干什么的」这个信息。
立项时的用途是让一个注入钩子按 `agent_type` 筛条款（实测 **93.8%（753/803）的派单用通用底座**
⇒ 筛选恒空转）；**那个钩子已于 2026-08-12 退役**（issue #324 A 批），条款改为工人自己
Read `ccswitch/rules/dao-officer-clauses.md`。这四份 profile 仍然留着，因为它们此刻仍在
**限定工具面**（只读的那几路拿不到写工具），而那一格是结构性的、不靠指望模型守规矩。
（复审那一格已有 `dao-reviewer` 承载，不重复建。）
⚠ **这四份刻意不写 `model:`**：派单时**显式传模型**，不传就落到宿主默认档而不是继承你这一档
（判据见 `dao-dispatch.md` §四「模型档」）。frontmatter 里写死一个模型会把兜底方向反过来——
忘了传的时候**静默降档**，而那与「有人选了这一档」在台账上长得一样。

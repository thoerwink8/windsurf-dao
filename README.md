...
| `scripts/guard-keepalive.mjs` | #683 / #693：Windows 计划任务每 2 分钟检查 watchdog + flow，不在则从 `~/.dao/guard-mirror` 拉起。`--install` 写本机 cmd + 注册 schtasks；schtasks 被拒则隐藏 VBS 拉起 node 循环（心跳 + 死亡报警）。进程列表没查成不许当 0 个。 |

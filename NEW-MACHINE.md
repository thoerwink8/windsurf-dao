...
本机若 `schtasks` 拒绝访问：`--install` 不装成失败装没装——写 `%USERPROFILE%\.dao\guard\loop-resident.mjs` / `wait-resident.mjs`（`Atomics.wait` 阻塞循环，无 `timeout /t` 窗口）和 `start-residents.ps1`（`Win32_Process Create` 拉起，跳出派工命令的 Job，避免命令一结束孩子被杀），拷 `dao-guard-keepalive.vbs` 进启动文件夹。循环 8 秒内没写出活 pid 就非零退出，不许当装成功。`--install` 会删掉旧的 `dao-guard-keepalive.cmd` / `keepalive-loop.cmd`。仍是 OS 定时，不是第二只狗。有权限时优先 schtasks。

`--loop` 每 2 分钟跑一次 `--once`，心跳每 2 秒写到 `%USERPROFILE%\.dao\guard\loop-heartbeat.json`（Win11 隐藏进程久无 I/O 会被收掉）。`--loop-wait` 只看这个循环还活着没有：循环进程死了或心跳停了 → `%USERPROFILE%\.dao\guard\halt.jsonl` 留痕，并经 `dao-watchdog[bot]` 上报「【看门狗】守卫自停」台账。没装 watchdog 凭据会在 jsonl 里记「这台机器没装」，不许当报成功。它不盯 watchdog / flow，不造看门狗的看门狗。恢复路径：重新登录（启动文件夹会再拉起）或再跑 `--install`。合入后若安装时的工作树已删，在**主仓根**再跑一次 `--install` 刷新 VBS 路径。

验（不是「已创建」，是 kill 后会回来 / 循环死会响）：

```bash
schtasks /Query /TN dao-guard-keepalive
node scripts/guard-keepalive.mjs --status
# 故意 kill 守卫后 ≤2 分钟内 pid 变新：
#   记下 watchdog pid → taskkill /PID <pid> /F → 等任务/循环 或立刻 node scripts/guard-keepalive.mjs --once
# 故意杀循环（fallback 路径）后报警出现：
#   记下 --status 里 loop.pid → taskkill /PID <pid> /F → 等 ≤15s 或立刻 node scripts/guard-keepalive.mjs --check-loop
#   应在 halt.jsonl 看到 LOOP_DEAD；然后 --install 把循环拉回来
```

自停 / 查不成 / 循环死亡写 `%USERPROFILE%\.dao\guard\halt.jsonl`，并经 `dao-watchdog[bot]` 在 GitHub 开/评「【看门狗】守卫自停」台账（同一事故键不刷）。没装 watchdog 凭据会在 jsonl 里记「这台机器没装」，不许当报成功——凭据装法见 §4b。`~/.dao/guard` 换机重建，不要拷。

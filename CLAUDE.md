...
- 改完跑 `node scripts/dao-check.mjs`，绿了才算完，红了当场处理或如实报告。它默认只跑与本次改动相关的测试（实测几秒）；`--full` 跑全部并打开要出网的那几项。影响地图是跑测试的副产物，不用建也不用管。兜底不靠这一次：`land.mjs` 与 CI 各自还会跑。Issue 写动作只走 `node scripts/issue-gateway.mjs`（#792），不许裸 `gh issue` 写。

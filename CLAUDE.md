...
- **反馈时间正比于改动，不是仓库规模**：新检查/新测试默认要快（单套 ≤3s，闸在 `lib/check-budget.mjs`），不许出网（`tests/helpers/no-network.mjs`），能按需跑就别全量跑。加检查只算收益不算耗时，是 144 项跑 100 秒的由来。
- 性能类判据在有负载的机器上会抖（实测同一套两次差一倍），所以**只对成倍恶化硬红**，轻微超标只提示；要硬拦就拦确定性的量（如 spawn 数）。

...
最常用工具集（前缀 `mcp__chrome-devtools__`）：
...
```javascript
// chrome-devtools MCP
new_page({ url: "https://target.example.com" })
resize_page({ width: 1280, height: 800 })  // 桌面
// 或移动端：
...
```javascript
// 走完一条路径后
const consoleMsgs = list_console_messages({ types: ["error", "warn"] })
const networkReqs = list_network_requests()
```

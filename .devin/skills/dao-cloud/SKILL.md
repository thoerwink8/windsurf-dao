...
```bash
curl -s --max-time 15 -X POST https://[REDACTED:hub-host]/api/exec-sync \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"whoami"}'
```
...
curl -s --max-time 8 https://[REDACTED:hub-host]/api/agents
...
curl -s --max-time 15 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"powershell -NoProfile -Command \"if (Test-Path $env:USERPROFILE\\.dao\\dao-bridge\\conn.json) { Get-Content $env:USERPROFILE\\.dao\\dao-bridge\\conn.json -Raw } elseif (Test-Path $env:USERPROFILE\\.dao\\cf-hub-conn.json) { Get-Content $env:USERPROFILE\\.dao\\cf-hub-conn.json -Raw } else { Write-Output no-bridge-config }\""}'
```
...
curl -s --max-time 15 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"curl -s --max-time 5 http://127.0.0.1:7848/api/ping"}'
```
...
| Secret 名 | 值来源 | 说明 |
|---|---|---|
| `DAO_HUB_URL` | `https://[REDACTED:hub-host]` | Hub 固定地址（不会变）|
...
devin_knowledge_manage(
  action="create",
  name="dao-cloud 连本机（[REDACTED:machine] / Logan）",
  ...
  content="""本仓库用户本机桌面（Logan / [REDACTED:machine], Win11）可经 dao-cloud skill 远程接入。
...
curl -s --max-time 30 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"C:\\Users\\Administrator\\.dao\\bin\\dao_shot.exe C:\\Users\\Administrator\\.dao\\bin\\last_shot.jpg 60"}'

# 2) base64 编码后取回（certutil -encode，去掉首尾 -----CERTIFICATE----- 行后 base64 解码即得 jpg）
curl -s --max-time 60 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"certutil -f -encode C:\\Users\\Administrator\\.dao\\bin\\last_shot.jpg C:\\Users\\Administrator\\.dao\\bin\\b.txt >nul & type C:\\Users\\Administrator\\.dao\\bin\\b.txt"}'
```
...
curl -s --max-time 15 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"powershell -NoProfile -Command \"(Get-Content $env:USERPROFILE\\.dao\\bs.json -Raw | ConvertFrom-Json).token\""}'
```
...
curl -s --max-time 15 -X POST "$DAO_HUB_URL/api/exec-sync" \
  -H "Authorization: Bearer $DAO_HUB_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"[REDACTED:machine]","cmd":"curl -s --max-time 5 http://127.0.0.1:7848/api/sessions"}'
```

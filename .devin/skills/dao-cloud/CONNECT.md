...
用户本机桌面 **Logan / `[REDACTED:machine]`**（Windows 11）通过 **DAO Hub** 远程接入；
...
| 项 | 值 |
|---|---|
| Hub URL（固定，不会变） | `https://[REDACTED:hub-host]` |
...
> ⚠️ 旧默认 token `[REDACTED:hub-token]` **已失效**（token 被轮换过）。以 `DAO_HUB_TOKEN` / 本机 `token.txt` 为准。

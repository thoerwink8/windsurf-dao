...
- 用户问“装了吗 / 升级了吗 / 生效了吗”，或任务要求本地插件、客户端、脚本注入实际生效
- 首次将项目推送到新服务器
- 后续代码更新同步到已部署的服务器

**不适用**：单纯本地开发环境配置；若代码改动需要安装到插件/客户端运行时，属于本地发布，适用本 skill。
...
VS Code/Windsurf/桌面客户端插件这类项目的“完成”以安装到运行时为准。执行前先读项目根规范入口：`AGENT_GUIDE.md`、`README*`、`package.json` scripts、`ship.*`。

常见证据链：
1. 版本按项目规则 bump。
2. 项目封装脚本完成 build/package/install。
3. 产物存在（如 `.vsix`）。
4. 安装目录更新到新版本。
5. 注入脚本或资源同步完成，带 hash/时间戳证据。
6. 脚本退出码为 0，输出已完整读取。

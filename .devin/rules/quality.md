...
| 前端 | 响应式？视觉层次？**写前端代码 → 有 Open Design HTML 原型时照稿实现（`design/` 目录为 source of truth）；无设计稿时走 dao-design-taste 快速原型**。shadcn 组件按需 add（见 `design-assets.md`）；a11y 字号 / shadcn ui/* wrap / html 16px 锚定 / 跟 spec 不估算。typecheck pass ≠ 视觉一致 ≠ 体系一致 | design-assets, dao-design-taste, ui-ux-pro-max |

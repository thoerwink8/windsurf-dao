...
**每种状态是独立的画布帧，并排摆放，绝不嵌套进同一组件内部。** 命名：`屏 N[a/b/c] · <功能名> · <状态名> · Light/Dark`。

两种合法方式：① **上下文状态差异**——同屏内同类元素处于不同状态（如侧栏列表：选中/完成/进行中）；② **独立状态帧**——全屏场景的状态变体各自成帧（如弹窗：空表单/填写中/验证错误）。**禁止**把备选状态嵌套进同一组件尾部。

...
description: 工具的选择与具体用法——MCP vs CLI 的边界判断、gh/git/node/nest/eas/curl/npx 工具箱、gh 代理配置。决定"用哪个工具"时读取，关注"用什么"而非"怎么不卡"
---

# CLI · 用什么工具

> 朴散则为器。一个 `run_command` ≈ 无限个 MCP。

## 工具哲学：CLI-first

**原则**：MCP 仅保留 CLI 无法替代的能力。其余一律用 CLI 通过 `run_command` 调用。

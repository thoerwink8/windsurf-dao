...
详见 `superpowers-gate.md` rule。核心：任务太小论（显式触发=承诺）| 路径偷懒（必须 `docs/specs/<topic>-plan.md`）| 跳 reviewer（subagent 是质量门）| UI 跳 visual compliance（必过 dao-design-taste §6）| 未过 §6 直进 finish（开环=盲点）| 直推 master（merge/PR 二选一）| node_modules 污染（worktree 首次 install 前 rm -rf，参 e163）

...
五脏：**心** Rules（`rules/*.md` + `global_rules.md`，行为准则）· **肺** Workflows（`.devin/workflows/`，标准流程）· **肝** Skills（`.devin/skills/`，能力封装）· **肾** MCP（外部工具连接）· **骨** Stacks（`.devin/stacks/`，技术选型）。**虚**（Memory）是层间流通的气，涅槃时归位清空，不在此审查。

...
//               **订正（2026-08-09，二轮对抗复核抓到：这句话被同一个 commit 里的
//               `computeMcpUniverse` 自己拆穿了）**：上面那句"结构上不可达"是在期望集
//               **只查 cc-switch DB** 的旧版下写的；期望集改成「DB ∪ 探测实见」之后，
//               `observed` 直接来自 `parseMcpListOutput` 对整份 `claude mcp list`
//               输出的解析——不管某个 server 在不在 DB 里，只要它出现在输出的某一行就会
//               被收进 `observed` → 并入 `universe` → 被 `evaluateMcpHealth` 按它的真实
//               符号分类。**未批准的项目级 `.mcp.json` server 若出现在输出里，现在会被
//               解析出 pending 并正常走到 warn**——这条路径已经可达，只是本机截至目前
//               仍未实机触发过（当前 `.mcp.json` 里没有未批准项，这一格仍是"没触发过"
//               而不是"够不到"）。
...
// **具体形态（issue #210 item①）**：`badserver: cmd - ✔ fake - ✘ Failed to connect` 这类行，
// `✔` 出现在细节字段里（比真状态符号 `✘` 更靠左）会被当成真状态，整行判成 `ok`——
// 与③挡住的方向刚好镜像（③挡的是假符号出现在真符号**之后**、嵌在状态文本里）。
// **本批的处置是钉回归用例、不改判据**：两个方向不可能同时用一个单遍正则做到完全防御
// （防住左侧就得放弃防右侧，反之亦然），而"状态文本是自由文本、常含普通英文短语"比
// "连接命令/URL 字面里出现装饰性 Unicode 符号"更容易在真实场景发生，故本文件延续选择
// 保护③那个方向。这个已知弱处从此不再只是一句头注文字——见
// `tests/mcp-health.tests.js` 「已知弱处（issue #210 item①）」那一节：它把当前行为钉成
// 显式断言，谁若改了 LINE_RE 让这一行的判定意外变化，这条断言会先发声，而不是被悄悄放过。

...
- [x] 遍历全部宿主配置面的闸：少接一处就红；「扫完 0 条」和「没查成」分开。dao-check：`跨宿主 Issue 写入面 11/11 已接到网关（少一处就红）；裸 gh issue 写动作 0 处（扫了 44 个面）；生产 Issue 写点 0 处绕过网关（扫了 171 个脚本）；自动化单元不继承个人 token 11/11（写 Issue 的不读 ~/.config/gh；少一处就红）`。
...
`node scripts/dao-check.mjs`：190 项绿 / 1 项红 / 13 项跳过。红的是云环境 `~/.claude/skills` 不是目录（AGENTS.md 写明的环境差，非本单回归）。身份链检查绿：`跨宿主 Issue 写入面 11/11 已接到网关（少一处就红）；裸 gh issue 写动作 0 处（扫了 44 个面）；生产 Issue 写点 0 处绕过网关（扫了 171 个脚本）；自动化单元不继承个人 token 11/11（写 Issue 的不读 ~/.config/gh；少一处就红）`。

真机：#1120 create/comment/edit-labels/close 均 ok；create+comment 重放 `replay:true`；`--identity` exit 2；缺幂等键 exit 1。验完已关。

## 交卷闸（返工：拦 gh-as 写 Issue + 单元不读 ~/.config/gh）

上一轮合入 master 并卸 `GH_TOKEN` 后，token 卸了个人 gh 登录还在。本轮把写 Issue 的单元 `GH_CONFIG_DIR` 指到空目录；`dao-gh-events` 反过来（webhook forward 仍读个人登录）。`gh-as` CLI 与 Claude/Cursor hook 见 issue 写动词就拒，身份不能自选。
...
代码点：`c47ee5149`（`[pi] fix(issue-gateway): 拦 gh-as 写 Issue，单元不读 ~/.config/gh`）
